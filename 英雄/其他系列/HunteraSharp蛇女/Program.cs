...
namespace Cassiopeia
{
    class Program
    {
        ...
        static Menu menu = new Menu("花边-Hunter蛇女", "Cassiopeia", true);
        ...
        static void OnGameLoad(EventArgs args)
        {
            ...
            menu.SubMenu("Harass").AddItem(new MenuItem("qHarass", "使用 Q").SetValue(true));
            menu.SubMenu("Harass").AddItem(new MenuItem("wHarass", "使用 W").SetValue(false));
            menu.SubMenu("Harass").AddItem(new MenuItem("eHarass", "使用 E").SetValue(true));

            menu.AddSubMenu(new Menu("清线", "LineClear"));
            menu.SubMenu("LineClear").AddItem(new MenuItem("lineclearQ", "使用 Q").SetValue(true));
            menu.SubMenu("LineClear").AddItem(new MenuItem("lineclearW", "使用 W").SetValue(false));
            menu.SubMenu("LineClear").AddItem(new MenuItem("onlylasthitE", "仅用E 补刀").SetValue(true));

            menu.AddSubMenu(new Menu("清野", "JungleClear"));
            menu.SubMenu("JungleClear").AddItem(new MenuItem("jungleclearQ", "使用 Q").SetValue(true));
            menu.SubMenu("JungleClear").AddItem(new MenuItem("jungleclearW", "使用 W").SetValue(false));

            menu.AddSubMenu(new Menu("大招", "Ultimate"));
            menu.SubMenu("Ultimate").AddItem(new MenuItem("comboR", "连招中使用R").SetValue(true));

            menu.SubMenu("Ultimate").SubMenu("AntiGapcloser").AddItem(new MenuItem("gapcloserR", "使用R防止突进").SetValue(false));
            menu.SubMenu("Ultimate").SubMenu("Interruptspells").AddItem(new MenuItem("interruptR", "使用R中断法术").SetValue(true));
            ...
            {
                menu.SubMenu("Ultimate").SubMenu("AntiGapcloser").AddItem(new MenuItem(enemyChamp.ChampionName + "gapcloser", "突进者 " + enemyChamp.ChampionName).SetValue(true));
                menu.SubMenu("Ultimate").SubMenu("Interruptspells").AddItem(new MenuItem(enemyChamp.ChampionName + "interrupt", "打断 " + enemyChamp.ChampionName).SetValue(true));
                if (enemyChamp.ChampionName == "Yasuo")
                    yasuoHere = true;
            }

            menu.AddSubMenu(new Menu("杂项", "Misc"));
            menu.SubMenu("Misc").AddItem(new MenuItem("castedalay", "使用 E 最小延迟 (点击)").SetValue(new Slider(0, 0, 2000)));
            menu.SubMenu("Misc").AddItem(new MenuItem("castedalay2", "使用 E 最大延迟 (点击)").SetValue(new Slider(0, 0, 2000)));
            menu.SubMenu("Misc").AddItem(new MenuItem("castWPoisoned", "使用W丨目标未中毒").SetValue(true));
            menu.SubMenu("Misc").AddItem(new MenuItem("focusSelectedTarget", "攻击选择目标").SetValue(false));
            menu.SubMenu("Misc").AddItem(new MenuItem("useAAcombo", "连招中使用 AA").SetValue(true));
            menu.SubMenu("Misc").AddItem(new MenuItem("PacketCast", "使用封包").SetValue(true));

            menu.AddSubMenu(new Menu("显示", "Drawings"));
            menu.SubMenu("Drawings").AddItem(new MenuItem("QRange", "显示 Q 范围").SetValue(new Circle(false, Color.FromArgb(150, Color.DodgerBlue))));
            menu.SubMenu("Drawings").AddItem(new MenuItem("ERange", "显示 E 范围").SetValue(new Circle(false, Color.FromArgb(150, Color.DodgerBlue))));
            menu.SubMenu("Drawings").AddItem(new MenuItem("RRange", "显示 R 范围").SetValue(new Circle(false, Color.FromArgb(150, Color.DodgerBlue))));
            menu.SubMenu("Drawings").SubMenu("DrawHP").AddItem(new MenuItem("drawHPDmg", "显示 冥火伤害").SetValue(true));
            menu.SubMenu("Drawings").SubMenu("DrawHP").AddItem(new MenuItem("qCountsDraw", "计算Q的伤害丨X秒").SetValue(new Slider(2, 1, 5)));
            menu.SubMenu("Drawings").SubMenu("DrawHP").AddItem(new MenuItem("eCountsDraw", "计算E的伤害丨X秒").SetValue(new Slider(4, 1, 10)));
            menu.SubMenu("Drawings").SubMenu("DrawHP").AddItem(new MenuItem("RCountsDraw", "计算R的伤害丨X秒").SetValue(true));

            menu.AddSubMenu(new Menu("信息", "message"));
            menu.SubMenu("message").AddItem(new MenuItem("Sprite", "Hunter蛇女"));
            menu.SubMenu("message").AddItem(new MenuItem("Hanhua", "汉化:花边下丶情未央"));
            menu.SubMenu("message").AddItem(new MenuItem("qqqun", "QQ群:299606556"));
