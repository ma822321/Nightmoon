...
namespace DZDraven_Reloaded
{
    class DZDraven_Reloaded
    {
        ...
        public static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Menu = new Menu("花边-Dz德莱文重做", "DravenReloaded", true);
            ...
            Menu.SubMenu("Axes").AddItem(new MenuItem("MaxAxeH", "骚扰时丨接斧头最大次数").SetValue(new Slider(2, 1, 4)));
            Menu.SubMenu("Axes").AddItem(new MenuItem("MaxAxeF", "清线时丨接斧头最大次数").SetValue(new Slider(2, 1, 4)));
            Menu.SubMenu("Axes").AddItem(new MenuItem("CatchRadius", "接斧头 半径").SetValue(new Slider(600, 200, 1000)));
            Menu.SubMenu("Axes").AddItem(new MenuItem("SafeZone", "接斧头 安全区域").SetValue(new Slider(125, 0, 325)));
            Menu.AddSubMenu(new Menu("杂 项", "Misc"));
            Menu.SubMenu("Misc").AddItem(new MenuItem("ManualR", "手动扔R 按键").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            ...
            Menu.SubMenu("Items").AddItem(new MenuItem("OwnHPercBotrk", "使用破败时丨自己血量").SetValue(new Slider(50, 1)));
            Menu.SubMenu("Items").AddItem(new MenuItem("EnHPercBotrk", "使用破败丨敌人血量").SetValue(new Slider(20, 1)));
            ...
            //Drawings Menu
            Menu.SubMenu("Drawing").AddItem(new MenuItem("DrawE", "显示 E 范围").SetValue(new Circle(true, Color.MediumPurple)));
