...
namespace Xerath_edit
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //Create the menu
            Config = new Menu("花边-xcsoft泽拉斯", ChampionName + "editedbyxcsoft", true);
            ...
            Config.SubMenu("R").AddItem(new MenuItem("rMode", "模式").SetValue(new StringList(new[] { "正常", "自定义", "按键" })));
            Config.SubMenu("R").AddItem(new MenuItem("rModeKey", "按键").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            Config.SubMenu("R").AddSubMenu(new Menu("延迟", "Custom delays"));
            for (int i = 1; i <= 3; i++)
                Config.SubMenu("R").SubMenu("Custom delays").AddItem(new MenuItem("Delay" + i, "Delay" + i).SetValue(new Slider(0, 1500, 0)));
            Config.SubMenu("R").AddItem(new MenuItem("PingRKillable", "本地ping提示可击杀目标").SetValue(true));
            ...
            Config.SubMenu("Farm")
                .AddItem(
                    new MenuItem("UseQFarm", "使用 Q").SetValue(
                        new StringList(new[] { "控线", "清兵", "全部", "不用" }, 2)));
            Config.SubMenu("Farm")
                .AddItem(
                    new MenuItem("UseWFarm", "使用 W").SetValue(
                        new StringList(new[] { "控线", "清兵", "全部", "不用" }, 1)));
            Config.SubMenu("Farm")
                .AddItem(
            ...
            Config.SubMenu("Misc").AddItem(new MenuItem("AutoEGC", "自动E反突进").SetValue(true));


            //Damage after combo:
            var dmgAfterComboItem = new MenuItem("DamageAfterR", "显示 3R 伤害").SetValue(true);
