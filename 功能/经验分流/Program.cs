...
namespace SharedExperience
{
    class Program
    {
        ...
        {
            menu = new Menu("花边-经验分流", "Shared Experience", true);
            menu.AddItem(new MenuItem("showEnemies", "显示多少人共享经验").SetValue(true));
            menu.AddItem(new MenuItem("onlyShowInv", "敌人不可见时丨仅显示文本").SetValue(false));
            menu.AddItem(new MenuItem("drawPredictionCircle", "有不可见敌人存在时丨显示预警线圈").SetValue(true));
            menu.AddItem(new MenuItem("invColor", "有不可见敌人存在丨文本颜色").SetValue(Color.FromArgb(255, 245, 25, 25)));
            menu.AddItem(new MenuItem("positionX", "文本 横坐标").SetValue(new Slider(142, -100, 200)));
            menu.AddItem(new MenuItem("positionY", "文本 纵坐标").SetValue(new Slider(21, -100, 100)));


            //Champion List Menu
            menu.AddSubMenu(new Menu("英雄 列表", "Champion List"));
            menu.SubMenu("Champion List").AddItem(new MenuItem("drawchampionlist", "显示 英雄 列表").SetValue(false));
            menu.SubMenu("Champion List").AddItem(new MenuItem("posX", "英雄 横坐标").SetValue(new Slider(Drawing.Width / 2, 0, Drawing.Width)));
            menu.SubMenu("Champion List").AddItem(new MenuItem("posY", "英雄 纵坐标").SetValue(new Slider(Drawing.Height / 2, 0, Drawing.Height)));
