    ...
    internal static class Program
    {
        ...
        private static void OnGameLoad(EventArgs args)
        {
            ...
            foreach (var entry in DefaultSpellSlotPriorities)
            {
                ...
                var subMenu = new Menu(entry.Key + " 设置", entry.Key + "extra");
                subMenu.AddItem(MakeSlider(entry.Key + "extra", "加点等级", 1, 1, 18));
                _menu.AddSubMenu(subMenu);
            }

            _activate = new MenuItem("activate", "启用等级").SetValue(new StringList(new[] { "2", "3", "4" }));
            _debug = new MenuItem("debug", "聊天框通知(本地)").SetValue(false);
            _delay = new MenuItem("delay", "加点延迟(毫秒)").SetValue(new Slider(0, 0, 2000));
