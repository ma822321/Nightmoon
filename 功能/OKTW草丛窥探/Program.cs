...
namespace OneKeyToBrain
{
    class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Config = new Menu("花边-OKTW草丛窥探", "OKTW Brain", true);
            Config.AddToMainMenu();

            Config.SubMenu("Iteams").AddItem(new MenuItem("pots", "使用 逃生").SetValue(true));
            Config.AddItem(new MenuItem("click", "显示 敌人 点击").SetValue(true));
            Config.AddItem(new MenuItem("infoCombo", "显示 连招 信息").SetValue(true));
            Config.SubMenu("Wards").AddItem(new MenuItem("ward", "敌人进草自动插眼").SetValue(false));
            Config.SubMenu("Wards").AddItem(new MenuItem("wardC", "仅连招使用").SetValue(false));
            Config.AddItem(new MenuItem("debug", "调试模式").SetValue(false));

            Config.SubMenu("Combo Key").AddItem(new MenuItem("Combo", "连招按键").SetValue(new KeyBind('t', KeyBindType.Press))); //32 == space
