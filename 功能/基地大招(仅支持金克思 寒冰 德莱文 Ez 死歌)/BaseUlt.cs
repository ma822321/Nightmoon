...
The idea where the lines come from is that u can calculate how far they are from base (enemySpawnPos) , so is it possible to make a script that can just show the position of the enemy while recalling i guess it would help , especially if u can show these lines even when they're not recalling (not sure if it's possible tho ) that would help so much in ganks and other stuff , thanks for your time , and i hope you give me your opinion about this script , have a nice day 
    ...
    internal class BaseUlt
    {
        ...
        public BaseUlt()
        {
            ...
            Menu.AddItem(new MenuItem("baseUlt", "基地 大招").SetValue(true));
            Menu.AddItem(new MenuItem("checkCollision", "检查 碰撞").SetValue(true));
            Menu.AddItem(new MenuItem("panicKey", "不够伤害也大招(按键)").SetValue(new KeyBind(32, KeyBindType.Press))); //32 == space
            Menu.AddItem(new MenuItem("regardlessKey", "取消时间限制(按键)").SetValue(new KeyBind(17, KeyBindType.Press))); //17 == ctrl
            ...
            TeamUlt = Menu.AddSubMenu(new Menu("队伍大招", "TeamUlt"));
            DisabledChampions = Menu.AddSubMenu(new Menu("禁止目标", "DisabledChampions"));

            if (compatibleChamp)
            {
                foreach (Obj_AI_Hero champ in Allies.Where(x => !x.IsMe && IsCompatibleChamp(x.ChampionName)))
                    TeamUlt.AddItem(new MenuItem(champ.ChampionName, "队友大招: " + champ.ChampionName).SetValue(false).DontSave());

                foreach (Obj_AI_Hero champ in Enemies)
                    DisabledChampions.AddItem(new MenuItem(champ.ChampionName, "禁止大招目标: " + champ.ChampionName).SetValue(false).DontSave());
            }

            var NotificationsMenu = Menu.AddSubMenu(new Menu("显示", "Notifications"));

            NotificationsMenu.AddItem(new MenuItem("notifRecFinished", "回城 完毕").SetValue(true));
            NotificationsMenu.AddItem(new MenuItem("notifRecAborted", "回城 中断").SetValue(true));
