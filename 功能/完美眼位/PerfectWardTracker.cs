...

{
  internal class PerfectWardTracker
  {
    ...
    public PerfectWardTracker()
    {
      ...
      //Create the menu
      Config = new LeagueSharp.Common.Menu("»¨±ß-ÍêÃÀÑÛÎ»", "PerfectWard", true);

      Config.SubMenu("Drawing").AddItem(new LeagueSharp.Common.MenuItem("drawplaces", "ÏÔÊ¾²åÑÛÎ»ÖÃ").SetValue(new Circle(true, System.Drawing.Color.FromArgb(100, 255, 0, 255))));
      Config.SubMenu("Drawing").AddItem(new LeagueSharp.Common.MenuItem("drawDistance", "²»ÏÔÊ¾Î»ÖÃ ¾àÀë >")).SetValue(new Slider(2000, 10000, 1));
      Config.SubMenu("Drawing").AddItem(new LeagueSharp.Common.MenuItem("placekey", "ÂÌÉ«ÑÛ °´¼ü").SetValue(new KeyBind("Z".ToCharArray()[0], KeyBindType.Press)));
      Config.SubMenu("Drawing").AddItem(new LeagueSharp.Common.MenuItem("placekeypink", "ÕæÑÛ °´¼ü").SetValue(new KeyBind("C".ToCharArray()[0], KeyBindType.Press)));
      Config.SubMenu("AutoBushRevealer").AddItem(new MenuItem("AutoBushKey", "²Ý´Ô¿úÌ½ °´¼ü").SetValue(new KeyBind("Ctrl".ToCharArray()[0], KeyBindType.Press)));

      Config.AddSubMenu(new Menu("²Ý´Ô¿úÌ½", "AutoBushRevealer"));
      
      Config.SubMenu("AutoBushRevealer").AddItem(new MenuItem("AutoBushEnabled", "ÆôÓÃ").SetValue(true));
