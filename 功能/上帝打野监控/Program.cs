...
namespace GodJungleTracker
{
    class Program
    {
        ...
        static void LoadMenu()
        {
            ...
            menu.SubMenu("Camp to Track").AddItem(new MenuItem("baron", "大龙").SetValue(true));
            menu.SubMenu("Camp to Track").AddItem(new MenuItem("dragon", "小龙").SetValue(true));
            menu.SubMenu("Camp to Track").AddItem(new MenuItem("blue", "蓝buff").SetValue(true));
            menu.SubMenu("Camp to Track").AddItem(new MenuItem("red", "红buff").SetValue(true));
            menu.SubMenu("Camp to Track").AddItem(new MenuItem("crab", "螃蟹").SetValue(true));
            menu.SubMenu("Camp to Track").AddItem(new MenuItem("raptor", "F4").SetValue(true));
            menu.SubMenu("Camp to Track").AddItem(new MenuItem("gromp", "石头人").SetValue(true));

            //Track List Menu
            menu.AddSubMenu(new Menu("监控设置", "Track List"));
            menu.SubMenu("Track List").AddItem(new MenuItem("drawtracklist", "显示 监控 信息").SetValue(false));
            menu.SubMenu("Track List").AddItem(new MenuItem("posX", "横坐标").SetValue(new Slider(Drawing.Width / 2, 0, Drawing.Width)));
            menu.SubMenu("Track List").AddItem(new MenuItem("posY", "纵坐标").SetValue(new Slider(Drawing.Height / 2, 0, Drawing.Height)));

            //Play Danger Sound
            menu.AddSubMenu(new Menu("游戏声音设置", "Play Danger Sound"));
            menu.SubMenu("Play Danger Sound").AddItem(new MenuItem("dragonsound", "小龙被攻击第一下").SetValue(true));
            menu.SubMenu("Play Danger Sound").AddItem(new MenuItem("dragonsoundtimes", "游戏开启多少分钟后").SetValue(new Slider(2, 1, 4)));
            menu.SubMenu("Play Danger Sound").AddItem(new MenuItem("baronsound", "大龙被攻击").SetValue(true));
            menu.SubMenu("Play Danger Sound").AddItem(new MenuItem("baronsoundtimes", "游戏开启多少分钟后").SetValue(new Slider(2, 1, 4)));
            menu.SubMenu("Play Danger Sound").AddItem(new MenuItem("sounddelay", "大龙声音 延迟 (s)").SetValue(new Slider(20, 1, 60)));
            menu.SubMenu("Play Danger Sound").AddItem(new MenuItem("soundfow", "仅用在无视野状态下").SetValue(false));
            menu.SubMenu("Play Danger Sound").AddItem(new MenuItem("soundscreen", "仅用在野怪不在屏幕显示范围内").SetValue(true));
            String [] volume = {"10%","25%","50%","75%","100%"};
            menu.SubMenu("Play Danger Sound").AddItem(new MenuItem("soundvolume", "声音大小").SetValue(new StringList(volume, 2)));

            //Track on Minimap
            menu.AddItem(new MenuItem("TrackonMinimap", "小地图显示").SetValue(true));

            menu.AddSubMenu(new Menu("信 息", "message"));
            menu.SubMenu("message").AddItem(new MenuItem("Sprite", "上帝打野监控"));
