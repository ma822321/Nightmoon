...
namespace BlackFeeder
{
    internal class Program
    {
        ...
        {
            Menu = new Menu("花边-Black送人头", "blackfeeder", true);

            // Feeding
            var feeder = new Menu("送人头设置", "blackfeeder");
            Menu.AddSubMenu(feeder);
            feeder.AddItem(new MenuItem("feedingActive", "启用!").SetValue(true));
            feeder.AddItem(new MenuItem("useSkillsActive", "使用技能去送!").SetValue(true));
            feeder.AddItem(new MenuItem("sayShitActive", "自动打字去说送人头!").SetValue(false));
            feeder.AddItem(new MenuItem("laughingActive", "Alt+4 嘲讽!").SetValue(true));
            feeder.AddItem(new MenuItem("buyItemsActive", "买Nike!").SetValue(true));
