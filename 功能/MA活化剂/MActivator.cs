...
namespace MasterActivator
{
    internal class MActivator
    {
        ...
        private void createMenuItem(MItem item, String parent, int defaultValue = 0, bool mana = false, int minManaPct = 0)
        {
            if (item.type == ItemTypeId.Ability || item.type == ItemTypeId.TeamAbility)
            {
                ...
                if (abilitySlot != SpellSlot.Unknown && abilitySlot == item.abilitySlot)
                {
                    ...
                    menu.AddItem(new MenuItem(item.menuVariable + "UseOnPercent", "使用丨 HP%")).SetValue(new Slider(defaultValue, 0, 100));
                    if (minManaPct > 0)
                    {
                        menu.AddItem(new MenuItem(item.menuVariable + "UseManaPct", "使用丨 最低MP%")).SetValue(new Slider(minManaPct, 0, 100));
                    }
                    var menuUseAgainst = new Menu("过滤器", "UseAgainst");
                    menuUseAgainst.AddItem(new MenuItem("tower" + item.menuVariable, "塔").SetValue(true));
                    menuUseAgainst.AddItem(new MenuItem("ignite" + item.menuVariable, "点燃").SetValue(true));
                    menuUseAgainst.AddItem(new MenuItem("king" + item.menuVariable, "破败").SetValue(false));
                    menuUseAgainst.AddItem(new MenuItem("basic" + item.menuVariable, "泉水").SetValue(false));
                    ...
                    if (enemyHero.Count() > 0)
                    {
                        foreach (Obj_AI_Hero hero in enemyHero)
                        {
                            ...
                            menuUseAgainstHero.AddItem(new MenuItem(item.menuVariable + hero.BaseSkinName, "启用").SetValue(true));
                            ...
                            menuUseAgainstHero.AddItem(new MenuItem("ignore" + item.menuVariable + hero.BaseSkinName, "忽略 %HP").SetValue(true));
            ...
            else if (item.type == ItemTypeId.KSAbility)
            {
                ...
                if (abilitySlot != SpellSlot.Unknown && abilitySlot == item.abilitySlot)
                {
                    ...
                    ksAbMenu.AddItem(new MenuItem(item.menuVariable, "启用").SetValue(true));
                    //choRMenu.AddItem(new MenuItem(choR.menuVariable + "plus", "Plus").SetValue(false));
                    ksAbMenu.AddItem(new MenuItem(item.menuVariable + "drawRange", "显示 范围").SetValue(true));
                    ksAbMenu.AddItem(new MenuItem(item.menuVariable + "drawBar", "显示 血条").SetValue(true));
            ...
            else
            {
                ...
                if (defaultValue != 0)
                {
                    ...
                    {
                        menu.AddItem(new MenuItem(item.menuVariable + "UseXUnits", "在 X 单位")).SetValue(new Slider(defaultValue, 1, 5));
                    }
                    else
                    {
                        menu.AddItem(new MenuItem(item.menuVariable + "UseOnPercent", "使用 " + (mana == false ? "%HP" : "%MP"))).SetValue(new Slider(defaultValue, 0, 100));
                    }
                }
                Config.SubMenu(parent).AddSubMenu(menu);
            }
        ...
        {
            Config = new Menu("花边-MA活化剂", "masterActivator", true);
            ...
            var menuSmiteSpell = new Menu("技能", "smiteSpell");
            ...
            menuSmiteDraw.AddItem(new MenuItem("dCamp", "野怪")).SetValue(true);
            menuSmiteDraw.AddItem(new MenuItem("justAS", "只选择野怪")).SetValue(false);
            ...
            Config.AddSubMenu(new Menu("防止 AOE", "offAOE"));
            ...
            Config.AddSubMenu(new Menu("回复", "regenerators"));
            ...
            var menu = new Menu("阿卡丽 W", "menuAkaliW");
            ...
            var message = new Menu("信 息", "message");
            message.AddItem(new MenuItem("Sprite", "脚本名字"));
            message.AddItem(new MenuItem("Hanhua", "汉化:花边下丶情未央"));
            message.AddItem(new MenuItem("qqqun", "QQ群:299606556"));
