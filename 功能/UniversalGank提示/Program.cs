...
namespace UniversalGankAlerter
{
    internal class Program
    {
        ...
        private void Game_OnGameLoad(EventArgs args)
        {
            ...
            _menu = new Menu("花边-Universal Gank提示", "universalgankalerter", true);
            _sliderRadius = new MenuItem("range", "最大 Ra范围nge").SetValue(new Slider(3000, 500, 5000));
            _sliderRadius.ValueChanged += SliderRadiusValueChanged;
            _sliderCooldown = new MenuItem("cooldown", "CD (秒)").SetValue(new Slider(10, 0, 60));
            _sliderLineDuration = new MenuItem("lineduration", "线上 时间(秒)").SetValue(new Slider(10, 0, 20));
            _dangerPing = new MenuItem("dangerping", "危险 提示 (本地)").SetValue(true);
