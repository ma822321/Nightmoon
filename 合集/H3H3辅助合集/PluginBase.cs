...
namespace Support
{
    ...
    public abstract class PluginBase
    {
        ...
        {
            Config = new Menu("花边汉化-辅助: " + Player.ChampionName, Player.ChampionName, true);
            ...
            ManaConfig = Config.AddSubMenu(new Menu("蓝量 限制", "Mana Limiter"));
            MiscConfig = Config.AddSubMenu(new Menu("杂 项", "Misc"));
            InterruptConfig = Config.AddSubMenu(new Menu("打 断", "Interrupt"));
            DrawingConfig = Config.AddSubMenu(new Menu("范 围", "Drawings"));

            // mana
            ManaConfig.AddSlider("HarassMana", "骚扰 蓝量比 %", 1, 1, 100);

            // misc
            MiscConfig.AddList("AttackMinions", "攻击 小兵?", new[] { "智能", "禁止", "总是" });
