...
namespace Support.Plugins
{
    ...
    public class Braum : PluginBase
    {
        ...
        public override void ComboMenu(Menu config)
        {
            ...
            config.AddSlider("Combo.R.Count", "使用R丨打击目标数", 2, 1, 5);
        }
        ...
        {
            config.AddBool("Misc.Shield.Skill", "使用盾 抵挡技能", true);
            config.AddBool("Misc.Shield.Target", "使用盾 抵挡目标", true);
            config.AddSlider("Misc.Shield.Health", "使用盾丨当前血量", 30, 1, 100);
        }

        public override void InterruptMenu(Menu config)
        {
            config.AddBool("Gapcloser.Q", "使用 Q 丨阻止突进", true);
            config.AddBool("Interrupt.R", "使用 R 丨打断法术", true);
        }
    }
}