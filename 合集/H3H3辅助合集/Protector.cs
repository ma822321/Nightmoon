...
namespace Support
{
    ...
    internal class Protector
    {
        ...
        {
            Menu = new Menu("花边-H3H3辅助合集", "Protector", true);

            // detector
            var detector = Menu.AddSubMenu(new Menu("探测器", "Detector"));

            // detector targeted
            var targeted = detector.AddSubMenu(new Menu("目 标", "Targeted"));
            targeted.AddItem(new MenuItem("TargetedActive", "启 用").SetValue(true));
            ...
            // detector skillshots
            var skillshot = detector.AddSubMenu(new Menu("技能 指向", "Skillshots"));
            skillshot.AddItem(new MenuItem("SkillshotsActive", "启 用").SetValue(true));
            skillshot.AddItem(new MenuItem("IsAboutToHitTime", "时间 延迟").SetValue(new Slider(200, 0, 400)));
            foreach (var ally in HeroManager.Allies)
            {
                skillshot.AddItem(
            ...
            // spells
            var spells = Menu.AddSubMenu(new Menu("法 术", "Spells"));
            ...
            // items
            var items = Menu.AddSubMenu(new Menu("物 品", "Items"));
            ...
            // misc
            var misc = Menu.AddSubMenu(new Menu("杂 项", "Misc"));
            misc.AddItem(new MenuItem("UsePackets", "使用 封包").SetValue(true));

            var messageMenu = Menu.AddSubMenu(new Menu("信 息", "message"));
            messageMenu.AddItem(new MenuItem("Sprite", "Beaving-死歌Sharp"));
