...
namespace MAC.Plugin
{
    internal class Vayne : PluginModel
    {
        ...
        public override void Misc(Menu config)
        {
            ...
            config.AddItem(new MenuItem("condemnNextAuto", "自动E触发第三下W被动").SetValue(false));
            config.AddItem(new MenuItem("minEnemiesInRangeR", "附近敌人数超过X丨自动R").SetValue(new Slider(2, 1, 5)));
            config.AddItem(new MenuItem("interruptE", "使用E打断").SetValue(true));
        ...
        public override void Extra(Menu config)
        {
            ...
            var MiscCSubMenu = new Menu("E设置", "MiscC");
            {
                MiscCSubMenu.AddItem(new MenuItem("PushDistance", "E推动距离").SetValue(new Slider(425, 400, 500)));
                MiscCSubMenu.AddItem(new MenuItem("CondemnTurret", "尝试把人订到塔下").SetValue(false));
                MiscCSubMenu.AddItem(new MenuItem("NoEEnT", "禁止把人E到敌人塔下").SetValue(true));
            }

            config.AddSubMenu(MiscCSubMenu);

            var MiscNoCondemn = new Menu("禁止E对象", "NoCondemn");
            {
        ...
        public override void Drawings(Menu config)
        {
            ...
            config.AddItem(new MenuItem("drawComboType", "显示 连招 模式").SetValue(true));
            config.AddItem(new MenuItem("drawCondemnPosition", "显示最好的E的位置").SetValue(true));
        }
    }
}
