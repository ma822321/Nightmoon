...
namespace MAC.Controller
{
    internal class ItemHandler : Utilitario
    {
        ...
        {
            config.AddItem(new MenuItem("igniteKill", "目标可击杀丨使用点燃").SetValue(true));
            config.AddItem(new MenuItem("igniteMinRange", "使用点燃最小范围").SetValue(new Slider(400, 0, 600)));
            config.AddItem(new MenuItem("igniteKS", "使用点燃击杀").SetValue(true));
