    ...
    class Syndra : Champion
    {
        ...
        private void LoadMenu()
        {
            var key = new Menu("按 键", "Key");
            {
                ...
                key.AddItem(new MenuItem("Misc_QE_Mouse2", "QE 鼠标位置", true).SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
                key.AddItem(new MenuItem("forceR", "自动R最好的目标", true).SetValue(new KeyBind("R".ToCharArray()[0], KeyBindType.Press)));
            ...
            var spellMenu = new Menu("技能 设置", "SpellMenu");
            {
                ...
                {
                    qMenu.AddItem(new MenuItem("Q_Auto_Immobile", "自动 Q (无法移动)", true).SetValue(true));
                    spellMenu.AddSubMenu(qMenu);
                }

                var wMenu = new Menu("W", "WMenu");
                {
                    wMenu.AddItem(new MenuItem("W_Only_Orb", "只捡起球", true).SetValue(false));
                    spellMenu.AddSubMenu(wMenu);
                }
                var rMenu = new Menu("R", "RMenu");
                {
                    rMenu.AddItem(new MenuItem("R_Overkill_Check", "检查 击杀", true).SetValue(true));

                    rMenu.AddSubMenu(new Menu("禁止R目标", "Dont_R"));
            ...
            {
                miscMenu.AddItem(new MenuItem("QE_Interrupt", "使用 QE丨打断技能", true).SetValue(true));
                miscMenu.AddItem(new MenuItem("E_Gap_Closer", "使用 E丨阻止突进", true).SetValue(true));
                miscMenu.AddItem(new MenuItem("smartKS", "使用 智能击杀系统", true).SetValue(true));
            ...
            var drawMenu = new Menu("范 围", "Drawing");
            {
                ...
                drawMenu.AddItem(new MenuItem("Draw_QE", "显示 QE", true).SetValue(true));
                ...
                drawMenu.AddItem(new MenuItem("Draw_QE_Line", "显示 QE 线条", true).SetValue(true));
                drawMenu.AddItem(new MenuItem("Draw_R_Killable", "标记 R可击杀", true).SetValue(true));
