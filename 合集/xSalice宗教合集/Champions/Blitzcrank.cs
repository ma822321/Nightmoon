    ...
    class Blitzcrank : Champion
    {
        ...
        private void LoadMenu()
        {
            ...
            var key = new Menu("按 键", "Keys"); {
                ...
                key.AddItem(new MenuItem("panic", "逃跑 按键(No 技能)",true).SetValue(new KeyBind("Z".ToCharArray()[0], KeyBindType.Press)));
                menu.AddSubMenu(key);
            }
            ...
            var qMenu = new Menu("Q", "qMenu");
            {
                ...
                qMenu.AddItem(new MenuItem("qSlow", "自动 Q (减速)",true).SetValue(true));
                qMenu.AddItem(new MenuItem("qImmobile", "自动 Q (无法移动)",true).SetValue(true));
                qMenu.AddItem(new MenuItem("qDashing", "自动 Q (突进)",true).SetValue(true));
            ...
            var combo = new Menu("连 招", "Combo");
            {
                ...
                combo.AddItem(new MenuItem("QE", "使用 E丨击飞", true).SetValue(true));
            ...
            {
                misc.AddItem(new MenuItem("UseInt", "使用 R 丨打断法术", true).SetValue(true));
                misc.AddItem(new MenuItem("resetE", "使用 E 丨重置平A", true).SetValue(true));
                misc.AddItem(new MenuItem("autoR", "使用 R 丨附近敌人数", true).SetValue(new Slider(3, 0, 5)));

                misc.AddSubMenu(new Menu("禁止 Q 对线", "intR"));
