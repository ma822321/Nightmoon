    ...
    class Akali : Champion
    {
        ...
        private void LoadMenu()
        {
            var key = new Menu("按 键", "Key");
            {
                ...
                key.AddItem(new MenuItem("LastHitQ", "使用Q丨补刀!",true).SetValue(new KeyBind("A".ToCharArray()[0], KeyBindType.Press)));
                //add to menu
                menu.AddSubMenu(key);
            }

            var spellMenu = new Menu("法术 设置", "SpellMenu");
            {
                var wMenu = new Menu("W", "WMenu");
                {
                    wMenu.AddItem(new MenuItem("useW_enemyCount", "使用 W丨附近敌人数",true).SetValue(new Slider(3, 1, 5)));
                    wMenu.AddItem(new MenuItem("useW_Health", "使用W丨血量低于 %",true).SetValue(new Slider(25)));
                    spellMenu.AddSubMenu(wMenu);
                }

                var eMenu = new Menu("E", "EMenu");
                {
                    eMenu.AddItem(new MenuItem("E_On_Killable", "使用E丨击杀",true).SetValue(true));
                    eMenu.AddItem(new MenuItem("E_Energy", "使用E丨能量>", true).SetValue(new Slider(100, 0, 200)));
                    spellMenu.AddSubMenu(eMenu);
                }

                var rMenu = new Menu("R", "RMenu");
                {
                    rMenu.AddItem(new MenuItem("R_Wait_For_Q", "先Q后R",true).SetValue(false));
                    rMenu.AddItem(new MenuItem("R_If_Killable", "使用R丨抢人头",true).SetValue(true));
                    rMenu.AddItem(new MenuItem("Dont_R_If", "禁止R丨敌人数>",true).SetValue(new Slider(3, 1, 5)));
                    rMenu.AddItem(new MenuItem("R_Min", "使用R丨最小距离", true).SetValue(new Slider(400, 50, 700)));
            ...
            {
                combo.AddItem(new MenuItem("Combo_mode", "连招 模式",true).SetValue(new StringList(new[] { "正常", "Q-延迟-R-AA-Q-AA"})));
                combo.AddItem(new MenuItem("Combo_Switch", "切换 模式",true).SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            ...
            var farm = new Menu("清 线", "LaneClear");
            {
                ...
                farm.AddItem(new MenuItem("LaneClear_useE_minHit", "使用 E (最少击中)", true).SetValue(new Slider(2, 1, 6)));
                //add to menu
            ...
            var drawMenu = new Menu("范 围", "Drawing");
            {
                ...
                drawMenu.AddItem(new MenuItem("Current_Mode", "显示 目前 模式",true).SetValue(true));
