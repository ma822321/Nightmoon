    ...
    class Lissandra : Champion
    {
        ...
        private void LoadMenu()
        {
            ...
            { 
                ...
                key.AddItem(new MenuItem("stunMelles", "眩晕敌人丨近战范围", true).SetValue(new KeyBind("M".ToCharArray()[0], KeyBindType.Toggle)));
                key.AddItem(new MenuItem("stunTowers", "眩晕敌人丨在塔下", true).SetValue(new KeyBind("J".ToCharArray()[0], KeyBindType.Toggle)));
                key.AddItem(new MenuItem("alwaysR", "开启 释放 R", true).SetValue(new KeyBind("H".ToCharArray()[0], KeyBindType.Toggle)));
                key.AddItem(new MenuItem("LastHitQQ", "Q补刀", true).SetValue(new KeyBind("A".ToCharArray()[0], KeyBindType.Press)));
                key.AddItem(new MenuItem("LaneClearActive", "打钱 按键!", true).SetValue(new KeyBind("Z".ToCharArray()[0], KeyBindType.Press)));
                menu.AddSubMenu(key);
            }
            //Combo menu:
            var combo = new Menu("连招 设置", "Combo");
            {
                ...
                combo.AddItem(new MenuItem("rHp", "使用R丨HP <", true).SetValue(new Slider(20)));
                combo.AddItem(new MenuItem("defR", "使用R保命丨敌人数>", true).SetValue(new Slider(3, 0, 5)));
                menu.AddSubMenu(combo);
            }

            //Harass menu
            var harass = new Menu("骚扰 设置", "Harass");
            {
            ...
            var misc = new Menu("杂项 设置", "Misc");
            {
                ...
                misc.AddItem(new MenuItem("UseHAM", "开启 释放E", true).SetValue(false));
                misc.AddItem(new MenuItem("UseEGap", "使用E丨突进目标", true).SetValue(true));
                misc.AddItem(new MenuItem("gapD", "最小 突进距离", true).SetValue(new Slider(600, 300, 1050)));
                menu.AddSubMenu(misc);
            }

            //Drawings menu:
            var drawing = new Menu("范围 设置", "Drawings");
            { 
                drawing.AddItem(new MenuItem("QRange", "Q 范围", true).SetValue(new Circle(false, Color.FromArgb(100, 255, 0, 255))));
                drawing.AddItem(new MenuItem("qExtend", "额外 Q 范围", true).SetValue(new Circle(false, Color.FromArgb(100, 255, 0, 255))));
                ...
                MenuItem drawFill = new MenuItem("Draw_Fill", "显示 满 连招 伤害", true).SetValue(new Circle(true, Color.FromArgb(90, 255, 169, 4)));
