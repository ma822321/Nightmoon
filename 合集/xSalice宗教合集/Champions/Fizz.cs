    ...
    class Fizz : Champion
    {
        ...
        private void LoadMenu()
        {
            ...
            var spellMenu = new Menu("技能 设置", "SpellMenu");
            {
                ...
                {
                    qMenu.AddItem(new MenuItem("Q_Min_Dist", "E 最小距离", true).SetValue(new Slider(100, 1, 475)));
                    spellMenu.AddSubMenu(qMenu);
                }

                var eMenu = new Menu("E", "EMenu");
                {
                    eMenu.AddItem(new MenuItem("E_Min_Dist", "E 最小距离", true).SetValue(new Slider(100, 1, 400)));
                    spellMenu.AddSubMenu(eMenu);
                }

                var rMenu = new Menu("R", "RMenu");
                {   
                    rMenu.AddItem(new MenuItem("rBestTarget", "释放 R 给最好的目标", true).SetValue(new KeyBind("R".ToCharArray()[0], KeyBindType.Press)));
                    rMenu.AddItem(new MenuItem("R_Delay", "释放R+Q的延迟(ms)", true).SetValue(new Slider(100, 1, 500)));
                    rMenu.AddItem(new MenuItem("R_Max_Dist", "R 最大距离", true).SetValue(new Slider(1000, 200, 1300)));
                    rMenu.AddItem(new MenuItem("ROverkill", "R 击杀检查", true).SetValue(false));
                    rMenu.AddItem(new MenuItem("AlwaysR", "连招时丨使用R", true).SetValue(true));
            ...
            {
                combo.AddItem(new MenuItem("Combo_mode", "连招 模式", true).SetValue(new StringList(new[] { "R-W-Q-E (Normal)", "W-Q-R-E(R During Q)", "R-E-W-Q (R->E gap)" }, 1)));
            ...
            var harass = new Menu("骚 扰", "Harass");
            {
                ...
                harass.AddItem(new MenuItem("turretCheck", "禁止塔下骚扰", true).SetValue(true));
                //add to menu
            ...
            var farm = new Menu("打 钱", "Farming");
            {
                ...
                farm.AddItem(new MenuItem("LaneClear_useE_minHit", "使用 E (击中小兵数)", true).SetValue(new Slider(2, 1, 6)));
                //add to menu
            ...
            var drawMenu = new Menu("范 围", "Drawing");
            {
                ...
                drawMenu.AddItem(new MenuItem("Draw_Mode", "显示 模式", true).SetValue(true));
