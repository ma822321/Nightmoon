    ...
    class Zed : Champion
    {
        ...
        private void LoadMenu()
        {
            var key = new Menu("按 键", "Key");
            {
                ...
                key.AddItem(new MenuItem("LastHitQ", "Q 补刀!", true).SetValue(new KeyBind("A".ToCharArray()[0], KeyBindType.Press)));
                key.AddItem(new MenuItem("Switch_1", "直线 模式", true).SetValue(new KeyBind("U".ToCharArray()[0], KeyBindType.Press)));
                key.AddItem(new MenuItem("Switch_2", "秀人一脸 模式", true).SetValue(new KeyBind("I".ToCharArray()[0], KeyBindType.Press)));
                key.AddItem(new MenuItem("Escape", "使用W丨向鼠标处逃跑", true).SetValue(new KeyBind("A".ToCharArray()[0], KeyBindType.Press)));
                //add to menu
                menu.AddSubMenu(key);
            }
            ...
            var spellMenu = new Menu("技能 设置", "SpellMenu");
            {
                ...
                {
                    wMenu.AddItem(new MenuItem("W_Require_QE", "QE技能CD好了丨自动W过去骚扰", true).SetValue(true));
                    wMenu.AddItem(new MenuItem("W_Follow_Combo", "直线连招丨跟随W释放", true).SetValue(false));
                ...
                {
                    rMenu.AddItem(new MenuItem("R_Place_line", "目标在直线上丨自动R", true).SetValue(new Slider(400, 250, 550)));
                    rMenu.AddItem(new MenuItem("R_Back", "敌人死亡后自动R影子回去", true).SetValue(true));
                    rMenu.AddItem(new MenuItem("useR_Health", "低血量丨自动R影子", true).SetValue(new Slider(10)));
            ...
            var combo = new Menu("连 招", "Combo");
            {
                ...
                combo.AddItem(new MenuItem("Combo_mode", "连招 模式", true).SetValue(new StringList(new[] { "正常", "直线连招", "秀人一脸", "用R不W", "正常(有R)" })));
                combo.AddItem(new MenuItem("Combo_Switch", "模式 切换", true).SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            ...
            var farm = new Menu("清 线", "LaneClear");
            {
                ...
                farm.AddItem(new MenuItem("LaneClear_useE_minHit", "使用 E (最小命中数)", true).SetValue(new Slider(2, 1, 6)));
                //add to menu
            ...
            var misc = new Menu("杂 项", "Misc");
            {
                ...
                misc.AddItem(new MenuItem("Use_W_KS", "使用 W 抢人头", true).SetValue(true));
                misc.AddItem(new MenuItem("AutoE", "自动 E (目标在范围内)", true).SetValue(true));
                //add to menu
            ...
            var drawMenu = new Menu("范 围", "Drawing");
            {
                ...
                drawMenu.AddItem(new MenuItem("Current_Mode", "显示 当前 模式", true).SetValue(true));
