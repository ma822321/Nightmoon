...
namespace Marksman
{
    internal class Urgot : Champion
    {
        ...
        public override bool ComboMenu(Menu config)
        {
            ...
            config.AddSubMenu(new Menu("大招 选项 1", "UltOpt1"));

            config.SubMenu("UltOpt1")
                .AddItem(
                    new MenuItem("UltOp1" + Id, "传送到友方 炮塔").SetValue(new KeyBind("T".ToCharArray()[0],
                        KeyBindType.Press)));

            config.AddSubMenu(new Menu("大招 选项 2", "UltOpt2"));
            config.SubMenu("UltOpt2")
                .AddItem(
                    new MenuItem("UltOp2" + Id, "传送到友方").SetValue(new KeyBind("G".ToCharArray()[0],
                        KeyBindType.Press)));
            config.SubMenu("UltOpt2")
                .AddItem(new MenuItem("UltOp2Count" + Id, "传送时丨最小友方数").SetValue(new Slider(1, 1, 5)));


            config.AddSubMenu(new Menu("不对某人使用大招", "DontUlt"));
        ...
        public override bool DrawingMenu(Menu config)
        {
            ...
            config.AddItem(
                new MenuItem("DrawQEx" + Id, "腐蚀 充能").SetValue(new Circle(true, Color.LightGray)));
