...
//                    .SetValue(ObjectManager.Player.Spellbook.GetSpell(SpellSlot.R).Name == "LucianR");
        ...
        public override bool ComboMenu(Menu config)
        {
            ...
            config.AddItem(new MenuItem("UseQExtendedC" + Id, "使用 Q 延伸").SetValue(true));
        ...
        public override bool HarassMenu(Menu config)
        {
            ...
            config.AddItem(
                new MenuItem("UseQTH" + Id, "使用 Q (自动)").SetValue(new KeyBind("T".ToCharArray()[0],
                    KeyBindType.Toggle)));
            config.AddItem(new MenuItem("Cx",""));
            config.AddItem(new MenuItem("UseQExtendedH" + Id, "使用 Q 延伸").SetValue(true));
            config.AddItem(
                new MenuItem("UseQExtendedTH" + Id, " 使用 Q 延伸 (自动)").SetValue(new KeyBind("H".ToCharArray()[0],
                    KeyBindType.Toggle)));
            config.AddItem(new MenuItem("Cx", ""));
        ...
        {
            config.AddItem(new MenuItem("Passive" + Id, "检查 被动").SetValue(true));
            return true;
        }
