...
namespace Marksman
{
    ...
    internal class Draven : Champion
    {
        ...
        public override bool DrawingMenu(Menu config)
        {
            ...
            config.AddItem(
                new MenuItem("DrawR", "R 范围(2000 距离)").SetValue(
                    new Circle(true, Color.FromArgb(100, 255, 0, 255))));
            config.AddItem(
                new MenuItem("DrawOrbwalk", "显示 走砍位置").SetValue(new Circle(true, Color.Yellow)));
            config.AddItem(new MenuItem("DrawReticles", "网线 标记").SetValue(new Circle(true, Color.Green)));
            return true;
        }

        public override bool MiscMenu(Menu config)
        {
            config.AddItem(new MenuItem("maxqamount", "最高 接斧头次数").SetValue(new Slider(2, 4, 1)));
            config.AddItem(new MenuItem("EGapCloser", "被突进 自动E").SetValue(true));
            config.AddItem(new MenuItem("EInterruptable", "自动E 打断法术").SetValue(true));
            config.AddItem(new MenuItem("RManualCast", "手动R(2000 范围)"))
                .SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press));
            config.AddItem(new MenuItem("Epeel", "E减速 逃走").SetValue(true));
            config.AddItem(new MenuItem("CatchRadius", "接斧头 半径").SetValue(new Slider(600, 200, 1000)));
            return true;
        }
