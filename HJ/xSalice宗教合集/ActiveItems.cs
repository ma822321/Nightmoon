...
namespace xSaliceReligionAIO
{
    class ActiveItems
    {
        ...
        public static void AddToMenu(Menu theMenu)
        {
            ...
            var offensiveItem = new Menu("进攻 物品", "Offensive Items");
            {
            ...
            var summoners = new Menu("召唤师 技能", "Summoners");
            {
                var ignite = new Menu("点燃", "Ignite");
                {
                    ignite.AddItem(new MenuItem("ignite", "使用 点燃", true).SetValue(true));
                    ignite.AddItem(
                        new MenuItem("igniteMode", "点燃 模式", true).SetValue(new StringList(new[] {"连招", "击杀"})));
        ...
        private static void AddOffensiveItem(Menu subMenu, ActiveItems item)
        {
            var active = new Menu(item.ActiveName, item.ActiveName);
            {
                ...
                active.AddItem(new MenuItem(item.ActiveName + "dmgCalc", "增加 伤害计算功能", true).SetValue(true));
                active.AddItem(new MenuItem(item.ActiveName + "killAble", "敌人可击杀时使用", true).SetValue(false));
                active.AddItem(new MenuItem(item.ActiveName + "always", "一直 开启", true).SetValue(item.Mode == 1 || item.Mode == 2));
                active.AddItem(new MenuItem(item.ActiveName + "myHP", "自己 HP <= %", true).SetValue(new Slider(25)));
                active.AddItem(new MenuItem(item.ActiveName + "enemyHP", "目标 HP <= %", true).SetValue(new Slider(50)));
