    ...
    class Velkoz : Champion
    {
        ...
        private void LoadMenu()
        {
            ...
            var key = new Menu("按 键", "Key");
            {
                ...
                key.AddItem(new MenuItem("R_Mouse", "R向鼠标", true).SetValue(new KeyBind("R".ToCharArray()[0], KeyBindType.Press)));
                //add to menu
                menu.AddSubMenu(key);
            }
            ...
            var spell = new Menu("技能 设置", "Spell");
            {
                ...
                var qMenu = new Menu("Q", "QSpell"); {
                    qMenu.AddItem(new MenuItem("qSplit", "自动Q", true).SetValue(true));
                    qMenu.AddItem(new MenuItem("qAngle", "角落Q", true).SetValue(true));
                    spell.AddSubMenu(qMenu);
                }   
                //R
                var rMenu = new Menu("R", "RSpell");
                {
                    rMenu.AddItem(new MenuItem("rAimer", "R目标", true).SetValue(new StringList(new[] { "自动", "鼠标" })));
                    rMenu.AddSubMenu(new Menu("禁止R对象", "DontUlt"));
            ...
            var draw = new Menu("范 围", "Drawings"); { 
                ...
                draw.AddItem(new MenuItem("drawUlt", "大招 击杀", true).SetValue(true));
