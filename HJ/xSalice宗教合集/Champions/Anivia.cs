    ...
    class Anivia : Champion
    {
        ...
        private void LoadMenu()
        {
            ...
            var key = new Menu("按 键", "Key");
            {
                ...
                key.AddItem(new MenuItem("escape", "奔跑吧 兄弟!",true).SetValue(new KeyBind("Z".ToCharArray()[0], KeyBindType.Press)));
                //add to menu
                menu.AddSubMenu(key);
            }
            ...
            var spellMenu = new Menu("技能 设置", "SpellMenu");
            {
                ...
                {
                    qMenu.AddItem(new MenuItem("qHit", "连招 命中",true).SetValue(new Slider(3, 1, 3)));
                    qMenu.AddItem(new MenuItem("qHit2", "骚扰 命中",true).SetValue(new Slider(3, 1, 4)));
                    qMenu.AddItem(new MenuItem("detonateQ", "自动 引爆 Q",true).SetValue(true));
                ...
                {
                    wMenu.AddItem(new MenuItem("wallKill", "挡住敌人方便击杀",true).SetValue(true));
                    spellMenu.AddSubMenu(wMenu);
                }

                var rMenu = new Menu("R", "RMenu");
                {
                    rMenu.AddItem(new MenuItem("checkR", "自动 关闭 R",true).SetValue(true));
            ...
            {
                misc.AddItem(new MenuItem("UseInt", "自动 打断法术", true).SetValue(true));
