    ...
    class Rumble : Champion
    {
        ...
        private void LoadMenu()
        {
            var key = new Menu("按 键", "Key");
            {
                ...
                key.AddItem(new MenuItem("HarassActive", "骚扰 爱看!", true).SetValue(new KeyBind("C".ToCharArray()[0], KeyBindType.Press)));
                ...
                key.AddItem(new MenuItem("LastHitE", "E 补刀!", true).SetValue(new KeyBind("A".ToCharArray()[0], KeyBindType.Press)));
                key.AddItem(new MenuItem("UseMecR", "智能完美R 按键", true).SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
                //add to menu
                menu.AddSubMenu(key);
            }
            ...
            var spellMenu = new Menu("技能 设置", "SpellMenu");
            {
                ...
                {
                    qMenu.AddItem(new MenuItem("Q_Auto_Heat", "使用 Q 控温", true).SetValue(true));
                    qMenu.AddItem(new MenuItem("Q_Over_Heat", "使用 Q 智能击杀", true).SetValue(true));
                    spellMenu.AddSubMenu(qMenu);
                }

                var wMenu = new Menu("W", "WMenu");
                {
                    wMenu.AddItem(new MenuItem("W_Auto_Heat", "使用 W 控温", true).SetValue(true));
                    wMenu.AddItem(new MenuItem("W_Always", "连招/骚扰 总使用W", true).SetValue(false));
                    wMenu.AddItem(new MenuItem("W_Block_Spell", "使用 W 抵挡伤害", true).SetValue(true));
                    spellMenu.AddSubMenu(wMenu);
                }

                var eMenu = new Menu("E", "EMenu");
                {
                    eMenu.AddItem(new MenuItem("E_Auto_Heat", "使用 E 控温", true).SetValue(false));
                    eMenu.AddItem(new MenuItem("E_Over_Heat", "使用 E 智能击杀", true).SetValue(true));
                    spellMenu.AddSubMenu(eMenu);
                }

                var rMenu = new Menu("R", "RMenu");
                {
                    rMenu.AddItem(new MenuItem("R_If_Enemy_Count", "自动 R丨敌人数 >=,6=关闭", true).SetValue(new Slider(4, 1, 6)));
                    rMenu.AddItem(new MenuItem("R_If_Enemy_Count_Combo", "连招使用R丨敌人数>,6=关闭", true).SetValue(new Slider(3, 1, 6)));
            ...
            var combo = new Menu("连 招", "Combo");
            {
                ...
                combo.AddItem(new MenuItem("qHit", "E 命中", true).SetValue(new Slider(3, 1, 3)));
            ...
            var harass = new Menu("骚 扰", "Harass");
            {
                ...
                harass.AddItem(new MenuItem("qHit2", "E 命中", true).SetValue(new Slider(3, 1, 4)));
                //add to menu
            ...
            {
                miscMenu.AddItem(new MenuItem("Stay_Danger", "站在危险地点", true).SetValue(new KeyBind("I".ToCharArray()[0], KeyBindType.Toggle)));
                miscMenu.AddItem(new MenuItem("E_Gap_Closer", "使用E 防突进", true).SetValue(true));
                //add to menu
            ...
            var drawMenu = new Menu("范 围", "Drawing");
            {
                ...
                drawMenu.AddItem(new MenuItem("Draw_R_Pred", "显示 R 最佳释放点", true).SetValue(true));
