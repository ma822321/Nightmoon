    ...
    class Orianna : Champion
    {
        ...
        private void LoadMenu()
        {
            ...
            var key = new Menu("按 键", "Keys"); { 
                ...
                key.AddItem(new MenuItem("escape", "奔跑吧 小发条!", true).SetValue(new KeyBind("Z".ToCharArray()[0], KeyBindType.Press)));
                //add to menu
                menu.AddSubMenu(key);
            }
            ...
            var spellMenu = new Menu("技能 设置", "SpellMenu");
            {
                ...
                var qMenu = new Menu("Q", "QSpell");{
                    qMenu.AddItem(new MenuItem("qHit", "Q 连招使用命中", true).SetValue(new Slider(3, 1, 3)));
                    qMenu.AddItem(new MenuItem("qHit2", "Q 骚扰使用命中", true).SetValue(new Slider(3, 1, 4)));
                    spellMenu.AddSubMenu(qMenu);
                }
                //W
                var wMenu = new Menu("W", "WSpell");
                {
                    wMenu.AddItem(new MenuItem("autoW", "W 命中", true).SetValue(new Slider(2, 1, 5)));
                    spellMenu.AddSubMenu(wMenu);
                }
                //E
                var eMenu = new Menu("E", "ESpell");
                {
                    eMenu.AddItem(new MenuItem("UseEDmg", "受到伤害自动E", true).SetValue(true));

                    eMenu.AddSubMenu(new Menu("E 保护友军", "shield"));
                    eMenu.SubMenu("shield").AddItem(new MenuItem("eAllyIfHP", "HP < %", true).SetValue(new Slider(40)));
                ...
                var rMenu = new Menu("R", "RSpell"); {
                    rMenu.AddItem(new MenuItem("autoR", "R 命中", true).SetValue(new Slider(3, 1, 5)));
                    rMenu.AddItem(new MenuItem("blockR", "没敌人禁止R", true).SetValue(true));
                    rMenu.AddItem(new MenuItem("overK", "击杀 检查", true).SetValue(true));
                    rMenu.AddItem(new MenuItem("killR", "手动R 按键", true).SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Toggle)));

                    rMenu.AddSubMenu(new Menu("自动使用R 目标", "intR"));
            ...
            var combo = new Menu("连 招", "Combo");
            {
                ...
                combo.AddItem(new MenuItem("autoRCombo", "使用R 丨命中数", true).SetValue(new Slider(2, 1, 5)));
            ...
            var farm = new Menu("打 钱", "Farm");
            {
                ...
                farm.AddItem(new MenuItem("qFarm", "只 Q/W(小兵数)", true).SetValue(new Slider(3, 0, 5)));
                menu.AddSubMenu(farm);
            }

            //intiator list:
            var initator = new Menu("先 手", "Initiator");
            {
            ...
            //Damage after combo:
            MenuItem dmgAfterComboItem = new MenuItem("DamageAfterCombo", "显示 连招 伤害", true).SetValue(true);
            ...
            var drawing = new Menu("范 围", "Drawings"); { 
                ...
                drawing.AddItem(new MenuItem("rModeDraw", "R 模式", true).SetValue(new Circle(true, Color.FromArgb(100, 255, 0, 255))));
