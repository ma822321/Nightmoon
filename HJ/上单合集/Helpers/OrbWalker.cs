    ...
    public static class Orbwalking
    {
        ...
        public class Orbwalker
        {
            ...
            public Orbwalker(Menu attachToMenu)
            {
                ...
                misc.AddItem(
                    new MenuItem("HoldPosRadius", "保持 半径 位置").SetShared().SetValue(new Slider(0, 0, 150)));
                misc.AddItem(new MenuItem("PriorizeFarm", "优先 补刀").SetShared().SetValue(true));
                _config.AddSubMenu(misc);
                /* Delay sliders */
                _config.AddItem(
                    new MenuItem("ExtraWindup", "额外时间").SetShared().SetValue(new Slider(80, 0, 200)));
                _config.AddItem(new MenuItem("FarmDelay", "补刀延迟").SetShared().SetValue(new Slider(0, 0, 200)));
                _config.AddItem(
                    new MenuItem("MovementDelay", "移动延迟").SetShared().SetValue(new Slider(80, 0, 150)))
                    .ValueChanged += (sender, args) => SetMovementDelay(args.GetNewValue<Slider>().Value);
                /*Load the menu*/
                _config.AddItem(
                    new MenuItem("LastHit", "补刀").SetShared().SetValue(new KeyBind('X', KeyBindType.Press)));

                _config.AddItem(new MenuItem("Farm", "打钱").SetShared().SetValue(new KeyBind('T', KeyBindType.Press)));
