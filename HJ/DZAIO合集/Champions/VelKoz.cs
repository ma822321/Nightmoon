...
namespace DZAIO.Champions
{
    class VelKoz : IChampion
    {
        ...
        public void OnLoad(Menu menu)
        {
            if (!DZAIO.IsDebug)
            {
                Game.PrintChat("閫欏€嬩篃鏄崐鎴愬搧-榄烽瓪鎬紒");
                return;
            }
            var cName = ObjectManager.Player.ChampionName;
            var comboMenu = new Menu("鱿鱼怪 - 连招", "dzaio.velkoz.combo");
            ...
            var skillOptionMenu = new Menu("技能 设置", "dzaio.velkoz.combo.skilloptions");
            comboMenu.AddSubMenu(skillOptionMenu);

            menu.AddSubMenu(comboMenu);
            var harrassMenu = new Menu("鱿鱼怪 - 骚扰", "dzaio.velkoz.harass");
