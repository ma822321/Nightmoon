...
namespace DZAIO.Champions
{
    class Caitlyn : IChampion
    {
        ...
        public void OnLoad(Menu menu)
        {
            ...
            var comboMenu = new Menu("女警 - 连招", "dzaio.caitlyn.combo");
            ...
            var skillOptionMenu = new Menu("技能 设置", "dzaio.caitlyn.combo.skilloptions");
            comboMenu.AddSubMenu(skillOptionMenu);

            menu.AddSubMenu(comboMenu);
            var harrassMenu = new Menu("女警 - 骚扰", "dzaio.caitlyn.harass");
            ...
            var farmMenu = new Menu("女警 - 打钱", "dzaio.caitlyn.farm");
            ...
            farmMenu.AddItem(
                new MenuItem("dzaio.caitlyn.farm.minminionsq", "使用Q丨最小小兵数").SetValue(new Slider(3, 1, 6)));
            menu.AddSubMenu(farmMenu);
            var miscMenu = new Menu("女警 - 杂项", "dzaio.caitlyn.misc");
            {
                miscMenu.AddItem(new MenuItem("dzaio.caitlyn.antigp", "E丨阻止突进").SetValue(true));
                miscMenu.AddItem(new MenuItem("dzaio.caitlyn.interrupt", "W丨打断法术").SetValue(true));
                miscMenu.AddItem(new MenuItem("dzaio.caitlyn.dashtomouse", "突进到鼠标 按键").SetValue(new KeyBind("S".ToCharArray()[0],KeyBindType.Press)));
                miscMenu.AddItem(new MenuItem("dzaio.caitlyn.manualr", "手动 R丨按键").SetValue(new KeyBind("Z".ToCharArray()[0], KeyBindType.Press)));
            }
            miscMenu.AddHitChanceSelector();
            menu.AddSubMenu(miscMenu);
            var drawMenu = new Menu("女警 - 范围", "dzaio.caitlyn.drawing");
