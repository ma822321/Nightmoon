...
namespace DZAIO.Champions
{
    class Graves : IChampion
    {
        ...
        public void OnLoad(Menu menu)
        {
            ...
            var comboMenu = new Menu("男枪 - 连招", "dzaio.graves.combo");
            ...
            var comboOptions = new Menu("技能 设置", "dzaio.graves.combo.skilloptions");
            {
                comboOptions.AddItem(new MenuItem("dzaio.graves.combo.minwenemiesc", "使用W丨命中敌人数").SetValue(new Slider(2, 1, 5)));
                comboOptions.AddItem(new MenuItem("dzaio.graves.combo.emaxrange", "E 距离").SetValue(new Slider(350, 1, 425)));
                comboOptions.AddItem(new MenuItem("dzaio.graves.combo.ecancel", "使用E 取消Q和R的后摇").SetValue(true));
            }
            comboMenu.AddSubMenu(comboOptions);
            menu.AddSubMenu(comboMenu);
            var harrassMenu = new Menu("男枪 - 骚扰", "dzaio.graves.harass");
            ...
            harrassMenu.AddItem(new MenuItem("dzaio.graves.harass.minwenemiesh", "使用W丨命中敌人数").SetValue(new Slider(2, 1, 5)));
            menu.AddSubMenu(harrassMenu);
            var farmMenu = new Menu("男枪 - 打钱", "dzaio.graves.farm");
            farmMenu.AddModeMenu(Mode.Farm, new[] { SpellSlot.Q }, new[] { true });
            farmMenu.AddManaManager(Mode.Farm, new[] { SpellSlot.Q }, new[] { 40 });
            menu.AddSubMenu(farmMenu);
            var miscMenu = new Menu("男枪 - 杂项", "GravesMisc");
            {
                miscMenu.AddItem(new MenuItem("dzaio.graves.misc.antigpw", "使用W丨阻止突进").SetValue(true));
                miscMenu.AddItem(new MenuItem("dzaio.graves.misc.antigpe", "使用E丨阻止突进").SetValue(true));
                miscMenu.AddItem(new MenuItem("dzaio.graves.misc.manualr", "手动 R丨按键").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            }
            miscMenu.AddHitChanceSelector();
            menu.AddSubMenu(miscMenu);

            var drawMenu = new Menu("男枪 - 范围", "dzaio.graves.drawings");
