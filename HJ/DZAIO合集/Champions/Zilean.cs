...
namespace DZAIO.Champions
{
    class Zilean : IChampion
    {
        ...
        public void OnLoad(Menu menu)
        {
            ...
            var comboMenu = new Menu("时光 - 连招", "dzaio.zilean.combo");
            ...
            var harrassMenu = new Menu("时光 - 骚扰", "dzaio.zilean.harass");
            ...
            var farmMenu = new Menu("时光 - 打钱", "dzaio.zilean.farm");
            farmMenu.AddModeMenu(Mode.Farm, new[] { SpellSlot.Q }, new[] { true });
            farmMenu.AddManaManager(Mode.Farm, new[] { SpellSlot.Q }, new[] { 40 });

            menu.AddSubMenu(farmMenu);

            var miscMenu = new Menu("时光 - 杂项", "dzaio.zilean.misc");
            {
                miscMenu.AddItem(new MenuItem("dzaio.champion.zilean.antigpe", "使用E丨阻止突进").SetValue(true));
                miscMenu.AddItem(new MenuItem("dzaio.champion.zilean.autoult", "自动 大招").SetValue(true));
                miscMenu.AddItem(new MenuItem("dzaio.champion.zilean.autoult.mana", "自动 大招丨最低蓝量 %").SetValue(new Slider(10)));
            }

            var drawMenu = new Menu("时光 - 范围", "dzaio.champion.zilean.drawings");
