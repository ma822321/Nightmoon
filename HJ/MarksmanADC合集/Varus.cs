...
namespace Marksman
{
    internal class Varus : Champion
    {
        ...
        {
            config.AddItem(
                new MenuItem("UseQC" + Id, "Q 模式").SetValue(new StringList(new[] { "关闭", "任何时刻", "最大 W 堆叠 = 3" }, 0)));
            config.AddItem(new MenuItem("UseEC" + Id, "使用 E").SetValue(true));
            config.AddItem(new MenuItem("UseRC" + Id, "使用 R").SetValue(true));

            config.AddItem(
                new MenuItem("QMinChargeC", "最小. Q 充能").SetValue(new Slider(1000, Q.ChargedMinRange,
                    Q.ChargedMaxRange)));
            config.AddItem(
                new MenuItem("UseQ2C" + Id, "使用 Insta Q").SetValue(new KeyBind("J".ToCharArray()[0], KeyBindType.Press)));
            return true;
        }
        ...
        {
            config.AddItem(new MenuItem("spellDelay" + Id, "法术 延迟").SetValue(new Slider(500, 0, 3000)));
            config.AddItem(
        ...
        public override bool DrawingMenu(Menu config)
        {
            ...
            config.AddItem(new MenuItem("DrawRS" + Id, "R:搜索范围").SetValue(new Circle(true, Color.White)));
