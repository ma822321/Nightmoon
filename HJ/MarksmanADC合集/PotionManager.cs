...
namespace Marksman
{
    internal class PotionManager
    {
        ...
        private void Load()
        {
            ...
            ExtrasMenu.AddSubMenu(new Menu("药剂 大师", "PotionManager"));

            ExtrasMenu.SubMenu("PotionManager").AddSubMenu(new Menu("生命", "Health"));
            ExtrasMenu.SubMenu("PotionManager")
                .SubMenu("Health")
                .AddItem(new MenuItem("HealthPotion", "使用 生命药剂").SetValue(true));
            ExtrasMenu.SubMenu("PotionManager")
                .SubMenu("Health")
                .AddItem(new MenuItem("HealthPercent", "嗑药 触发百分比").SetValue(new Slider(30)));

            ExtrasMenu.SubMenu("PotionManager").AddSubMenu(new Menu("蓝量", "Mana"));
            ExtrasMenu.SubMenu("PotionManager")
                .SubMenu("Mana")
                .AddItem(new MenuItem("ManaPotion", "使用 补蓝药剂").SetValue(true));
            ExtrasMenu.SubMenu("PotionManager")
                .SubMenu("Mana")
                .AddItem(new MenuItem("ManaPercent", "嗑药 触发百分比").SetValue(new Slider(30)));
