...
namespace Marksman
{
    internal class Vayne : Champion
    {
        ...
        public override bool ComboMenu(Menu config)
        {
            ...
            config.AddItem(new MenuItem("FocusW" + Id, "自动锁定敌人").SetValue(true));
            return true;
        }
        ...
        {
            config.AddItem(new MenuItem("UseET" + Id, "使用 E (自动)").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Toggle)));
            config.AddItem(new MenuItem("UseEInterrupt" + Id, "使用 E 打断").SetValue(true));
            config.AddItem(new MenuItem("PushDistance" + Id, "E 推动距离").SetValue(new Slider(425, 475, 300)));
            config.AddItem(new MenuItem("CompleteSilverBuff" + Id, "Q破轮子吗的E").SetValue(true));
            return true;
        }
