...
namespace Tracker
{
    ...
    public static class HbTracker
    {
        ...
        {
            Config = menu.AddSubMenu(new Menu("CD 检测", "CD Tracker"));
            Config.AddItem(new MenuItem("TrackAllies", "检测 友军").SetValue(true));
            Config.AddItem(new MenuItem("TrackEnemies", "检测 敌人").SetValue(true));
            Config.AddItem(new MenuItem("TrackMe", "检测 自己").SetValue(false));
        }
