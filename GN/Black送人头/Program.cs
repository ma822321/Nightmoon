...
//
    ...
    //        Game.Say("/all Good game guys, well played.");
     //   }
        ...
        {
            Menu = new Menu("花边汉化-Black送人头", "blackfeeder", true);

            // Feeding
            var feeder = new Menu("花边汉化-送人头设置", "blackfeeder");
            Menu.AddSubMenu(feeder);
            feeder.AddItem(new MenuItem("feedingActive", "开 送!").SetValue(true));
            feeder.AddItem(new MenuItem("useSkillsActive", "使用技能去送!").SetValue(true));
            feeder.AddItem(new MenuItem("sayShitActive", "边ALL嘲讽边送(已取消此功能)!").SetValue(true));
            feeder.AddItem(new MenuItem("laughingActive", "送的时候Alt+4!").SetValue(true));
            feeder.AddItem(new MenuItem("buyItemsActive", "买东西去送(鞋子之类的加移速的玩意)!").SetValue(true));
