...
namespace Advanced_Turn_Around
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad()
        {
            ...
            Variable.Config = new Menu("花边汉化-转身躲技能", "ATA", true);

            Variable.Config.AddItem(new MenuItem("Enabled", "开 启").SetValue(true));

            Variable.Config.AddSubMenu(new Menu("英雄-法术", "CAS"));
            foreach (var champ in Variable.ExistingChampions)
            {
                Variable.Config.SubMenu("CAS")
                    .AddSubMenu(new Menu(champ.CharName + "的法术", champ.CharName));
                Variable.Config.SubMenu("CAS")
            ...
            Game.PrintChat("鑺辫竟姹夊寲-杞韩韬叉妧鑳戒辅鍔犺浇鎴愬姛!");
