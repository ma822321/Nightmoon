...
namespace SpellNamesandSlots
{
    internal class Program
    {
        ...
        public static void Game_OnGameLoad(EventArgs args)
        {
            try
            {
                menu = new Menu("Sala模式", "dmg", true);
                menu.AddItem(
                    new MenuItem("range", "范围 检查").SetValue(new Slider(1500, 300, 20000)));
                menu.AddItem(
                    new MenuItem("gthan2", "if > 2 敌人 CD 检查 (安全)").SetValue(new Slider(2, 0, 8)));
                menu.AddItem(
                    new MenuItem("lthan2", "if <= 2 敌人 CD 检查 (安全)").SetValue(new Slider(1, 0, 8)));
                menu.AddItem(
                    new MenuItem("theircd", "敌人 CD 检查 (<=2)").SetValue(new Slider(4, 0, 10)));
                menu.AddItem(
                    new MenuItem("theircd2", "敌人 CD 检查 (>2)").SetValue(new Slider(4, 0, 10)));
                menu.AddToMainMenu();
            ...
            //alldmg = new double();
            Game.PrintChat("鍗曟寫sala妯″紡 - 鍔犺浇鎴愬姛!");
