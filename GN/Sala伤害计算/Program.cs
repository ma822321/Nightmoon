...
namespace SpellNamesandSlots
{
    internal class Program
    {
        ...
        public static void Game_OnGameLoad(EventArgs args)
        {
            try
            {
                menu = new Menu("群殴模式", "dmg", true);
                menu.AddItem(
                    new MenuItem("range", "范围 检查").SetValue(new Slider(1500, 300, 20000)));
                menu.AddItem(
                    new MenuItem("gthan2", "> 2 敌人 CD").SetValue(new Slider(2, 0, 8)));
                menu.AddItem(
                    new MenuItem("lthan2", "<= 2 敌人 CD").SetValue(new Slider(1, 0, 8)));
                menu.AddItem(
                    new MenuItem("theircd", "敌人 CD 检查").SetValue(new Slider(4, 0, 10)));
                menu.AddItem(
