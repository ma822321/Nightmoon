...
namespace SAssemblies.Activators
{
    internal class AutoSmite
    {
        ...
        public static Menu.MenuItemSettings AutoSmiteActivator = new Menu.MenuItemSettings(typeof(AutoSmite));
        ...
        {
            var menu = new LeagueSharp.Common.Menu("SAutoSmite", "SAssembliesSActivatorsAutoSmite", true);
            SetupMenu(menu);
            menu.AddToMainMenu();
        }
        ...
        {
            AutoSmiteActivator.Menu = menu.AddSubMenu(new LeagueSharp.Common.Menu(Language.GetString("ACTIVATORS_AUTOSMITE_MAIN"), "SAssembliesActivatorsAutoSmite"));
            AutoSmiteActivator.MenuItems.Add(
                AutoSmiteActivator.Menu.AddItem(
                    new MenuItem("SAssembliesActivatorsAutoSmiteSmallCampsActive", Language.GetString("ACTIVATORS_AUTOSMITE_SMALL_CAMPS")).SetValue(false)));
            AutoSmiteActivator.MenuItems.Add(
                AutoSmiteActivator.Menu.AddItem(
                    new MenuItem("SAssembliesActivatorsAutoSmiteAutoSpell", Language.GetString("ACTIVATORS_AUTOSMITE_AUTO_SPELL")).SetValue(false)));
            AutoSmiteActivator.MenuItems.Add(
                AutoSmiteActivator.Menu.AddItem(
                    new MenuItem("SAssembliesActivatorsAutoSmiteKeyActive", Language.GetString("GLOBAL_KEY")).SetValue(new KeyBind(78, KeyBindType.Toggle))));
            AutoSmiteActivator.MenuItems.Add(
                AutoSmiteActivator.Menu.AddItem(new MenuItem("SAssembliesActivatorsAutoSmiteActive", Language.GetString("GLOBAL_ACTIVE")).SetValue(false)));
            return AutoSmiteActivator;
        }
                ...
                if (Vector3.Distance(ObjectManager.Player.ServerPosition, minion.ServerPosition) < 1500)
                {
                    foreach (string monster in _monsters)
                    {
                        if (minion.SkinName == monster && minion.IsVisible)
                        {
                            ...
                            health =
                                !AutoSmiteActivator.GetMenuItem("SAssembliesActivatorsAutoSmiteKeyActive")
                                    .GetValue<KeyBind>()
                                    .Active
                                    ? health + " Disabled"
                                    : health;
        ...
        {
            return Activator.Activators.GetActive() && AutoSmiteActivator.GetActive();
        }
