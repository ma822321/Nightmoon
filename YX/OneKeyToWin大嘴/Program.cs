...
namespace KogMaw
{
    class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //Create the menu
            Config = new Menu("花边-OneKeyToWin大嘴", ChampionName, true);
            ...
            Config.SubMenu("E config").AddItem(new MenuItem("AGC", "被突进时自动E").SetValue(true));

            Config.SubMenu("W config").AddItem(new MenuItem("autoW", "自动 W").SetValue(true));
            Config.SubMenu("W config").AddItem(new MenuItem("harasW", "骚扰 W").SetValue(true));

            Config.SubMenu("R option").AddItem(new MenuItem("autoR", "自动 R").SetValue(true));
            Config.SubMenu("R option").AddItem(new MenuItem("comboStack", "连招丨最大 R堆叠").SetValue(new Slider(2, 5, 0)));
            Config.SubMenu("R option").AddItem(new MenuItem("harasStack", "骚扰丨最大 R堆叠").SetValue(new Slider(1, 5, 0)));
            Config.SubMenu("R option").AddItem(new MenuItem("Rcc", "R 状态").SetValue(true));
            Config.SubMenu("R option").AddItem(new MenuItem("Rslow", "R 减速").SetValue(true));
            ...
            Config.SubMenu("Draw").AddItem(new MenuItem("onlyRdy", "显示 R可击杀目标").SetValue(true));
            Config.SubMenu("Draw").AddItem(new MenuItem("orb", "走砍 目标").SetValue(true));

            Config.AddItem(new MenuItem("Hit", "技能 命中").SetValue(new Slider(2, 3, 0)));
