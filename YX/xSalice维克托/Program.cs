...
namespace ViktorTheMindBlower
{
    class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //Create the menu
            menu = new Menu("花边汉化-xSalice维克托", ChampionName, true);

            //Orbwalker submenu
            var orbwalkerMenu = new Menu("走 砍", "my_Orbwalker");
            ...
            menu.SubMenu("Keys").AddItem(new MenuItem("ComboActive", "连 招!").SetValue(new KeyBind("C".ToCharArray()[0], KeyBindType.Press)));
            menu.SubMenu("Keys").AddItem(new MenuItem("HarassActive", "骚 扰!").SetValue(new KeyBind("S".ToCharArray()[0], KeyBindType.Press)));
            menu.SubMenu("Keys").AddItem(new MenuItem("HarassActiveT", "骚 扰 (切换)!").SetValue(new KeyBind("Y".ToCharArray()[0], KeyBindType.Toggle)));
            menu.SubMenu("Keys").AddItem(new MenuItem("LaneClearActive", "打 钱!").SetValue(new KeyBind("X".ToCharArray()[0], KeyBindType.Press)));
            menu.SubMenu("Keys").AddItem(new MenuItem("LastHitQQ", "使用Q丨最后一下").SetValue(new KeyBind("A".ToCharArray()[0], KeyBindType.Press)));

            //Spell Menu
            menu.AddSubMenu(new Menu("技能 设置", "Spell"));

            //Q Menu
            menu.SubMenu("Spell").AddSubMenu(new Menu("Q 设置", "QSpell"));
            menu.SubMenu("Spell").SubMenu("QSpell").AddItem(new MenuItem("QAARange", "目标在平A范围内丨只会Q").SetValue(true));
            menu.SubMenu("Spell").SubMenu("QSpell").AddItem(new MenuItem("autoAtk", "目标在平A范围内丨先A后Q").SetValue(true));
            //W Menu
            menu.SubMenu("Spell").AddSubMenu(new Menu("W 设置", "WSpell"));
            menu.SubMenu("Spell").SubMenu("WSpell").AddItem(new MenuItem("wSlow", "自动W丨减速").SetValue(true));
            menu.SubMenu("Spell").SubMenu("WSpell").AddItem(new MenuItem("wImmobile", "自动W丨固定").SetValue(true));
            menu.SubMenu("Spell").SubMenu("WSpell").AddItem(new MenuItem("wDashing", "自动W丨迅速").SetValue(true));
            menu.SubMenu("Spell").SubMenu("WSpell").AddItem(new MenuItem("useW_Hit", "连招命中时丨自动W").SetValue(new Slider(2, 1, 5)));
            //R
            menu.SubMenu("Spell").AddSubMenu(new Menu("R 设置", "RSpell"));
            menu.SubMenu("Spell").SubMenu("RSpell").AddItem(new MenuItem("useR_Hit", "连招命中时丨自动R").SetValue(new Slider(2, 1, 5)));
            menu.SubMenu("Spell").SubMenu("RSpell").AddItem(new MenuItem("rAlways", "连招时丨总使用R").SetValue(new KeyBind("K".ToCharArray()[0], KeyBindType.Toggle)));

            //Combo menu:
            menu.AddSubMenu(new Menu("连 招", "Combo"));
            menu.SubMenu("Combo").AddItem(new MenuItem("selected", "集中 选择目标").SetValue(true));
            ...
            menu.SubMenu("Combo").AddItem(new MenuItem("eHit", "E 命中!").SetValue(new Slider(3, 1, 4)));
            menu.SubMenu("Combo").AddItem(new MenuItem("UseRCombo", "使用 R").SetValue(true));
            menu.SubMenu("Combo").AddItem(new MenuItem("ignite", "使用 Ignite").SetValue(true));
            menu.SubMenu("Combo")
                .AddItem(new MenuItem("igniteMode", "模式:").SetValue(new StringList(new[] { "连招", "击杀" }, 0)));
            ...
            menu.SubMenu("Harass").AddItem(new MenuItem("eHit2", "E 命中!").SetValue(new Slider(4, 1, 4)));
            ...
            menu.SubMenu("Misc").AddItem(new MenuItem("UseInt", "使用R丨打断法术").SetValue(true));
            menu.SubMenu("Misc").AddItem(new MenuItem("packet", "使用 封包").SetValue(true));

            //Damage after combo:
            var dmgAfterComboItem = new MenuItem("DamageAfterCombo", "显示 连招伤害").SetValue(true);
            ...
            menu.SubMenu("Drawings")
                .AddItem(new MenuItem("E2Range", "拓展 范围").SetValue(new Circle(false, Color.FromArgb(100, 255, 0, 255))));
            menu.SubMenu("Drawings")
            ...
            Game.PrintChat(ChampionName + " Loaded! --- by xSalice 花边汉化-xSalice维克托 加载成功!");
        }
