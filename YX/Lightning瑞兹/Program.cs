...
namespace LightningRyze
{
	internal class Program
	{
		...
		private static void Game_OnGameLoad(EventArgs args)
		{
			...
			Config = new Menu("花边汉化-Lightning瑞兹", "Lightning Ryze", true);
			...
			Config.SubMenu("Combo").AddItem(new MenuItem("TypeCombo", "连招 模式:").SetValue(new StringList(new[] {"混合 模式","爆发 模式","消耗 模式"},0)));
			...
			Config.SubMenu("Harass").AddItem(new MenuItem("ManaH", "MP >%丨开启骚扰").SetValue(new Slider(30, 1, 100)));
			...
			Config.SubMenu("Extra").AddItem(new MenuItem("tearStack", "Q+W 双重眼泪效果").SetValue(new KeyBind("Z".ToCharArray()[0], KeyBindType.Toggle)));
			Config.SubMenu("Extra").AddItem(new MenuItem("UseSeraphs", "使用 炽天使之拥").SetValue(true));
			Config.SubMenu("Extra").AddItem(new MenuItem("HP", "当 HP <=% 使用↑").SetValue(new Slider(20, 100, 0)));
			Config.SubMenu("Extra").AddItem(new MenuItem("YasuoWall", "亚索开启风墙时丨不使用技能").SetValue(true));
			Config.SubMenu("Extra").AddItem(new MenuItem("WInterrupt", "使用W丨打断法术").SetValue(true));
			Config.SubMenu("Extra").AddItem(new MenuItem("WGap", "使用W丨阻止突进").SetValue(true));
			...
			Game.PrintChat("花边汉化-Lightning瑞兹 加载成功!");
