...
namespace Aatrox
{
    static class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Game.PrintChat("鑺辫竟姹夊寲-Aureus鍓戦瓟鍔犺浇鎴愬姛!");
        }
...
//                    var rdmg = R.GetDamage(enemy);
        ...
        {
            _config = new Menu("花边汉化-Aureus剑魔", ChampionName, true);

            var targetSelectorMenu = new Menu("目标 选择", "TargetSelector");
            TargetSelector.AddToMenu(targetSelectorMenu);
            _config.AddSubMenu(targetSelectorMenu);

            _orbwalker = new Orbwalking.Orbwalker(_config.AddSubMenu(new Menu("走 砍", "Orbwalker")));
            ...
            _config.SubMenu("Combo").AddItem(new MenuItem("useQ", "使用 Q")).SetValue(true);
            _config.SubMenu("Combo").AddItem(new MenuItem("useW", "使用 W")).SetValue(true);
            _config.SubMenu("Combo")
                .AddItem(new MenuItem("useWHealth", "使用 W 治疗"))
                .SetValue(new Slider(35, 1, 100));
            _config.SubMenu("Combo")
                .AddItem(new MenuItem("useE", "使用 E"))
                .SetValue(true);
            _config.SubMenu("Combo")
                .AddItem(new MenuItem("useR", "对" + R.Instance.SData.Name + "使用R"))
                .SetValue(true);
            _config.SubMenu("Combo").AddItem(new MenuItem("useItems", "使用 物品")).SetValue(true);

            // Harass
            _config.AddSubMenu(new Menu("骚 扰", "Harass"));
            _config.SubMenu("Harass")
                .AddItem(new MenuItem("harassMode", "骚扰 模式"))
                .SetValue(new StringList(new[] {"E", "E + Q"}, 1));
            _config.SubMenu("Harass")
                .AddItem(new MenuItem("harassQ", "对 " + Q.Instance.SData.Name + " 使用(Q)"))
                .SetValue(true);
            _config.SubMenu("Harass")
                .AddItem(new MenuItem("harassE", "对 " + E.Instance.SData.Name + "使用(E)"))
                .SetValue(true);
            _config.SubMenu("Harass")
                .AddItem(new MenuItem("harassHealth", "骚扰最低血量比"))
                .SetValue(new Slider(50, 1, 100));

            // Items
            _config.AddSubMenu(new Menu("物 品", "Items"));
            _config.SubMenu("Items").AddItem(new MenuItem("useItems", "使用 物品")).SetValue(true);
            _config.SubMenu("Items").AddItem(new MenuItem("BOTRK", "破 败")).SetValue(true);
            _config.SubMenu("Items").AddItem(new MenuItem("OMEN", "兰 顿")).SetValue(true);
            _config.SubMenu("Items")
                .AddItem(new MenuItem("omenCount", "使用兰顿丨最小敌人数"))
                .SetValue(new Slider(1, 1, 5));
            _config.SubMenu("Items").AddItem(new MenuItem("HYDRA", "九头蛇")).SetValue(true);
            _config.SubMenu("Items").AddItem(new MenuItem("GBLADE", "幽梦")).SetValue(true);

            // Lane Clear / Jungle Clear
            _config.AddSubMenu(new Menu("清 线", "LaneClear"));
            _config.SubMenu("LaneClear").AddItem(new MenuItem("laneQ", "使用 Q")).SetValue(true);
            _config.SubMenu("LaneClear").AddItem(new MenuItem("laneE", "使用 E")).SetValue(true);
            _config.SubMenu("LaneClear")
                .AddItem(new MenuItem("laneHealth", "清线最低血量比"))
                .SetValue(new Slider(50, 1, 100));

            // Killsteal
            _config.AddSubMenu(new Menu("抢人头", "Killsteal"));
            _config.SubMenu("Killsteal").AddItem(new MenuItem("killSteal", "使用 智能模式")).SetValue(true);
            _config.SubMenu("Killsteal").AddItem(new MenuItem("autoIgnite", "自动 点燃")).SetValue(true);

            // Misc
            _config.AddSubMenu(new Menu("封 包", "Packets"));
            _config.SubMenu("Packets").AddItem(new MenuItem("usePackets", "使用 封包")).SetValue(true);

            // Drawing
            _config.AddSubMenu(new Menu("范围", "Drawing"));
            _config.SubMenu("Drawing")
                .AddItem(new MenuItem("drawQ", "显示 Q 范围"))
                .SetValue(new Circle(true, Color.FromArgb(255, 255, 0, 0)));
            _config.SubMenu("Drawing")
                .AddItem(new MenuItem("drawE", "显示 E 范围"))
                .SetValue(new Circle(true, Color.FromArgb(255, 255, 0, 0)));
            _config.SubMenu("Drawing").AddItem(new MenuItem("DamageIndicator", "伤害 指示").SetValue(true));
