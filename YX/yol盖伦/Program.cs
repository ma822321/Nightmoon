...
namespace yol0Garen
{
    class Program
    {
        ...
        private static void OnLoad(EventArgs args)
        {
            ...
            _menu = new Menu("花边汉化-yol盖伦", "yol0Garen", true);
            _orbwalker = new Orbwalking.Orbwalker(_menu.AddSubMenu(new Menu("走砍 设置", "Orbwalker")));
            _menu.AddSubMenu(new Menu("目标 选择", "Target Selector"));
            TargetSelector.AddToMenu(_menu.SubMenu("Target Selector"));

            _menu.AddSubMenu(new Menu("连招 设置", "Combo"));
            _menu.AddSubMenu(new Menu("清线 设置", "LaneClear"));
            _menu.AddSubMenu(new Menu("杂项 设置", "Misc"));
            _menu.AddSubMenu(new Menu("物品 设置", "Item"));
            _menu.AddSubMenu(new Menu("击杀 设置", "KS"));
            _menu.AddSubMenu(new Menu("范围 设置", "Drawing"));

            _menu.SubMenu("Combo").AddItem(new MenuItem("UseQ", "使用 Q").SetValue(true));
            _menu.SubMenu("Combo").AddItem(new MenuItem("UseE", "使用 E").SetValue(true));
            _menu.SubMenu("Combo").AddItem(new MenuItem("UseR", "使用 R").SetValue(true));
            _menu.SubMenu("Combo").AddItem(new MenuItem("UseIgnite", "使用 点燃丨可击杀").SetValue(true));
            _menu.SubMenu("Combo").AddItem(new MenuItem("UseItems", "使用 物品").SetValue(true));
            _menu.SubMenu("Combo").AddItem(new MenuItem("yol0Mode", "锁定 目标").SetValue(true));

            _menu.SubMenu("LaneClear").AddItem(new MenuItem("UseE", "使用 智能E 定位").SetValue(true));

            _menu.SubMenu("Misc").AddItem(new MenuItem("Interrupt", "使用 Q丨打断法术").SetValue(true));

            _menu.SubMenu("Item").AddItem(new MenuItem("UseTiamat", "提亚马特").SetValue(true));
            _menu.SubMenu("Item").AddItem(new MenuItem("UseHydra", "九头蛇").SetValue(true));
            _menu.SubMenu("Item").AddItem(new MenuItem("UseGhostblade", "鬼刀").SetValue(true));
            _menu.SubMenu("Item").AddItem(new MenuItem("UseBilgewater", "水银刀").SetValue(true));
            _menu.SubMenu("Item").AddItem(new MenuItem("UseBlade", "破败").SetValue(true));
            _menu.SubMenu("Item").AddItem(new MenuItem("UseHextech", "科技枪").SetValue(true));
            _menu.SubMenu("Item").AddItem(new MenuItem("UseRanduins", "兰顿").SetValue(true));

            _menu.SubMenu("KS").AddItem(new MenuItem("KsQ", "Q").SetValue(false));
            _menu.SubMenu("KS").AddItem(new MenuItem("KsE", "E").SetValue(false));
            _menu.SubMenu("KS").AddItem(new MenuItem("KsR", "R").SetValue(true));
            _menu.SubMenu("KS").AddItem(new MenuItem("KsItems", "物品").SetValue(true));
            _menu.SubMenu("KS").AddItem(new MenuItem("KsI", "点燃").SetValue(true));

            _menu.SubMenu("Drawing").AddItem(new MenuItem("drawTarget", "显示 目标").SetValue(new Circle(true, Color.Red)));
            _menu.SubMenu("Drawing").AddItem(new MenuItem("drawERange", "显示 E 范围").SetValue(new Circle(true, Color.Green)));
            _menu.SubMenu("Drawing").AddItem(new MenuItem("drawRRange", "显示 R 范围").SetValue(new Circle(true, Color.Cyan)));
            _menu.SubMenu("Drawing").AddItem(new MenuItem("drawKillability", "显示 击杀").SetValue(true));
            _menu.SubMenu("Drawing").AddItem(new MenuItem("drawDamage", "显示 伤害").SetValue(true));

            Utility.HpBarDamageIndicator.DamageToUnit += GetComboDamage;
            Utility.HpBarDamageIndicator.Color = Color.GreenYellow;
        ...
        {
            Utility.HpBarDamageIndicator.Enabled = _menu.SubMenu("Drawing").Item("drawDamage").GetValue<bool>();
        ...
        private static void OnDraw(EventArgs args)
        {
            ...
            {
                foreach (var enemy in ObjectManager.Get<Obj_AI_Hero>().Where(unit => unit.IsEnemy && unit.IsVisible && !unit.IsDead))
                {
                    var wts = Drawing.WorldToScreen(enemy.Position);
                    if (GetComboDamage(enemy) > enemy.Health)
                    {
                        Drawing.DrawText(wts[0] - 20, wts[1] + 20, Color.Red, "鑽変粬!!");
                    }
                    else
                    {
                        Drawing.DrawText(wts[0] - 20, wts[1] + 20, Color.White, "鑽変笉姝讳粬!");
                    }
                }
            }
        }
        ...
        private static void Combo(Obj_AI_Hero target)
        {
            ...
            {
                _orbwalker.SetMovement(false);
                Player.IssueOrder(GameObjectOrder.MoveTo, target.Path[0]);
            }
        ...
        private static void OnInterruptibleTarget(Obj_AI_Hero sender, Interrupter2.InterruptableTargetEventArgs args)
        {
