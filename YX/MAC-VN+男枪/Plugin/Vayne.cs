...
namespace MAC.Plugin
{
    internal class Vayne : PluginModel
    {
        ...
        {
            config.AddItem(new MenuItem("comboType", "连招 模式:").SetValue(new StringList(new[] { "正常", "Advanced", "Gosu" }, 2)));
            config.AddItem(new MenuItem("condemnNextAuto", "下次攻击时自动E").SetValue(false));
            config.AddItem(new MenuItem("minEnemiesInRangeR", "使用大招丨敌人数").SetValue(new Slider(2, 1, 5)));
            config.AddItem(new MenuItem("interruptE", "使用 E丨打断法术").SetValue(true));
            config.AddItem(new MenuItem("gapcloser", "反突进").SetValue(true));
            config.AddItem(new MenuItem("gapcloserE", "使用E丨反突进").SetValue(true));
        }

        public override void Extra(Menu config)
        {
            var MiscMSubMenu = new Menu("杂项-蓝量管理", "MiscM");
            {
                MiscMSubMenu.AddItem(new MenuItem("saveMana", "连招时保存 % 蓝量").SetValue(new Slider(50, 0, 100)));
            }

            config.AddSubMenu(MiscMSubMenu);

            var MiscCSubMenu = new Menu("杂项 - 谴责", "MiscC");
            {
                MiscCSubMenu.AddItem(new MenuItem("PushDistance", "E 推动距离").SetValue(new Slider(425, 400, 500)));
                MiscCSubMenu.AddItem(new MenuItem("CondemnTurret", "尝试把人E到塔下").SetValue(false));
                MiscCSubMenu.AddItem(new MenuItem("NoEEnT", "不把人E回他们塔下").SetValue(true));
            }

            config.AddSubMenu(MiscCSubMenu);

            var MiscNoCondemn = new Menu("禁止 谴责", "NoCondemn");
            {
        ...
        {
            config.AddItem(new MenuItem("disableAll", "禁止 所有范围").SetValue(false));
            config.AddItem(new MenuItem("drawQ", "Q 范围").SetValue(true));
            config.AddItem(new MenuItem("drawE", "E 范围").SetValue(true));
            config.AddItem(new MenuItem("drawComboType", "显示连招伤害").SetValue(true));
            config.AddItem(new MenuItem("drawCondemnPosition", "显示 最好E的位置").SetValue(true));
        }
    }
}
