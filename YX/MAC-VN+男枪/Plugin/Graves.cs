...
namespace MAC.Plugin
{
    internal class Graves : PluginModel
    {
        ...
        {
            config.AddItem(new MenuItem("comboType", "连招 模式").SetValue(new StringList(new[] { "正 常", "先 进", "混 合" }, 2)));
            config.AddItem(new MenuItem("dontETurr", "敌人在塔下丨禁止E").SetValue(true));
        }
        ...
        public override void Extra(Menu config)
        {
            ...
            {
                MiscMSubMenu.AddItem(new MenuItem("saveMana", "连招时 保持在 % 以上").SetValue(new Slider(50, 0, 100)));
            }
        ...
        {
            config.AddItem(new MenuItem("disableAll", "禁用 所有范围").SetValue(false));
            config.AddItem(new MenuItem("drawQ", "Q 范围").SetValue(true));
            config.AddItem(new MenuItem("drawW", "W 范围").SetValue(true));
            config.AddItem(new MenuItem("drawE", "E 范围").SetValue(true));
            config.AddItem(new MenuItem("drawR", "R 范围").SetValue(true));
            config.AddItem(new MenuItem("drawComboType", "显示 连招伤害").SetValue(true));
        }
    }
}
