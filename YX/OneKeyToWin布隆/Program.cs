...
namespace OneKeyToBrain
{
    class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Config = new Menu("花边-OneKeyToWin布隆", "OKTW Brain", true);
            Config.AddToMainMenu();

            Config.SubMenu("Iteams").AddItem(new MenuItem("pots", "使用 药水").SetValue(true));
            Config.AddItem(new MenuItem("click", "显示 敌人 状态(被动)").SetValue(true));
            ...
            Config.SubMenu("Wards").AddItem(new MenuItem("wardC", "仅 连招启用").SetValue(false));
            Config.AddItem(new MenuItem("debug", "调试 模式").SetValue(false));

            Config.SubMenu("Combo Key").AddItem(new MenuItem("Combo", "连招 按键").SetValue(new KeyBind('32', KeyBindType.Press))); //32 == space
