...
namespace SigmaSeries.Plugins
{
    public class Cassiopeia : PluginBase
    {
        ...
        public override void FarmMenu(Menu config)
        {
            ...
            config.AddItem(new MenuItem("useEFarm", "使用 E").SetValue(new StringList(new[] { "控线", "清线", "两者", "都不" }, 2)));
            config.AddItem(new MenuItem("JungleActive", "清 野!").SetValue(new KeyBind("C".ToCharArray()[0], KeyBindType.Press)));
