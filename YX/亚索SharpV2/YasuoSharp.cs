...
namespace YasuoSharpV2
{
        internal class YasuoSharp
        {
            ...
            private static void onLoad(EventArgs args)
            {
                ...
                Game.PrintChat("鑺辫竟姹夊寲-浜氱储SharpV2.1鍔犺浇鎴愬姛!");
                try
                {

                    Config = new Menu("花边汉化-亚索SharpV2", "Yasuo", true);
                    //Orbwalker
                    ...
                    Config.SubMenu("combo").AddItem(new MenuItem("comboItems", "使用 Items")).SetValue(true);
                    //SmartR
                    Config.SubMenu("combo").AddItem(new MenuItem("smartR", "智能 R")).SetValue(true);
                    Config.SubMenu("combo").AddItem(new MenuItem("useRHit", "使用 R丨打击目标")).SetValue(new Slider(3, 5, 1));
                    Config.SubMenu("combo").AddItem(new MenuItem("useRHitTime", "使用 R丨可击杀时")).SetValue(true);
                    Config.SubMenu("combo").AddItem(new MenuItem("useEWall", "使用 E丨安全时")).SetValue(true);
                    //Flee away
                    Config.SubMenu("combo").AddItem(new MenuItem("flee", "E逃跑")).SetValue(new KeyBind('Z', KeyBindType.Press, false));
                    Config.SubMenu("combo").AddItem(new MenuItem("fleeStack", "逃跑时攥Q")).SetValue(true);


                    //LastHit
                    Config.AddSubMenu(new Menu("补 刀", "lHit"));
                    ...
                    //LaneClear
                    Config.AddSubMenu(new Menu("清 线", "lClear"));
                    Config.SubMenu("lClear").AddItem(new MenuItem("useQlc", "使用 Q")).SetValue(true);
                    Config.SubMenu("lClear").AddItem(new MenuItem("useEmpQHit", "风击中最小数")).SetValue(new Slider(3, 6, 1));
                    Config.SubMenu("lClear").AddItem(new MenuItem("useElc", "使用 E")).SetValue(true);
                    //Harass
                    Config.AddSubMenu(new Menu("骚 扰", "harass"));
                    Config.SubMenu("harass").AddItem(new MenuItem("harassTower", "塔下 骚扰")).SetValue(false);
                    Config.SubMenu("harass").AddItem(new MenuItem("harassOn", "骚扰 敌人")).SetValue(true);
                    Config.SubMenu("harass").AddItem(new MenuItem("harQ3Only", "仅使用第三段Q 骚扰")).SetValue(false);
                    //Drawings
                    Config.AddSubMenu(new Menu("范 围", "drawing"));
                    Config.SubMenu("drawing").AddItem(new MenuItem("disDraw", "禁止 所有 范围")).SetValue(false);
                    ...
                    Config.SubMenu("drawing").AddItem(new MenuItem("drawWJ", "显示 风墙")).SetValue(true);

                    //Extra
                    Config.AddSubMenu(new Menu("额 外", "extra"));
                    Config.SubMenu("extra").AddItem(new MenuItem("djTur", "禁止 跳到塔下")).SetValue(true);
                    Config.SubMenu("extra").AddItem(new MenuItem("autoLevel", "自动 加点")).SetValue(true);
                    Config.SubMenu("extra").AddItem(new MenuItem("levUpSeq", "加点模式:")).SetValue(new StringList(new string[2] { "Q E W Q 开始", "Q E Q W 开始" }));

                    //LastHit
                    Config.AddSubMenu(new Menu("智能 风墙", "aShots"));
                    //SmartW
                    Config.SubMenu("aShots").AddItem(new MenuItem("smartW", "智能 W")).SetValue(true);
                    Config.SubMenu("aShots").AddItem(new MenuItem("smartEDogue", "使用E丨躲避技能")).SetValue(true);
                    Config.SubMenu("aShots").AddItem(new MenuItem("wwDanger", "使用W丨仅危险")).SetValue(false);
                    Config.SubMenu("aShots").AddItem(new MenuItem("wwDmg", "使用W丨血量比")).SetValue(new Slider(0, 100, 1));
                    skillShotMenu = getSkilshotMenu();
                    Config.SubMenu("aShots").AddSubMenu(skillShotMenu);
                    //Debug
                    Config.AddSubMenu(new Menu("调 试", "debug"));
                    Config.SubMenu("debug").AddItem(new MenuItem("WWLast", "绘制 最后W的位置")).SetValue(new KeyBind('T', KeyBindType.Press, false));
                    Config.SubMenu("debug").AddItem(new MenuItem("saveDash", "保存 突进")).SetValue(new KeyBind('O', KeyBindType.Press, false));
                    Config.SubMenu("debug").AddItem(new MenuItem("exportDash", "开发 dashes")).SetValue(new KeyBind('P', KeyBindType.Press, false));
                    Config.SubMenu("debug").AddItem(new MenuItem("deleteDash", "删除 最后 突进")).SetValue(new KeyBind('I', KeyBindType.Press, false));
