...
namespace Prince_Urgot
{
    internal class ComboClass
    {
        ...
        {
            ComboMenu.AddSubMenu(new Menu("连 招", "combo"));
            ...
            ComboMenu.SubMenu("combo").AddItem(new MenuItem("useR", "使用 R (自动R到塔下)")).SetValue(true);
            ComboMenu.SubMenu("combo").AddItem(new MenuItem("preE", "E 命中率").SetValue(new StringList((new[] { "低", "中", "高", "非常 高" }))));

            ComboMenu.AddSubMenu(new Menu("杂 项", "misc"));
            ComboMenu.SubMenu("misc").AddItem(new MenuItem("autoInt", "打断技能" + "R").SetValue(false));
            ComboMenu.SubMenu("misc").AddItem(new MenuItem("packetcast", "使用 范围内公布")).SetValue(true);
        }
        #endregion
