...
namespace Prince_Urgot
{
    internal class ItemClass
    {
        ...
        {
            ItemMenu.AddSubMenu(new Menu("物 品", "items"));
            ItemMenu.SubMenu("items").AddItem(new MenuItem("useMura", "使用 魔宗利刃").SetValue(true));
            ItemMenu.SubMenu("items").AddItem(new MenuItem("useIgnite", "使用 骚扰").SetValue(true));
        }
    }
}
